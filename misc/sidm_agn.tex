...
as dissipative (dSIDM), inelastic (iSIDM), and DM--baryon
interacting (IDM) variants.  All runs include identical sub-grid
physics: metal-dependent cooling, star formation with a polytropic
equation of state, and dual-mode (thermal plus kinetic) AGN feedback
from sink-particle SMBHs.
...
We perform eight simulations from identical initial conditions, varying
the dark matter self-interaction model:
...
\code{dSIDM} & 1   & constant & Dissipative, $f_{\rm diss} = 0.5$ \\
\code{IDM}   & 1   & constant & DM--baryon drag, $\sigma_{\rm db}/m = 0.01$ \\
\code{iSIDM} & 1   & constant & Inelastic (exo), $\delta = 100\,\mathrm{keV}$, $f_{\rm exc,0} = 0.3$ \\
\code{iSIDM-endo} & 1 & constant & Inelastic (endo), $\delta = 100\,\mathrm{keV}$, $f_{\rm exc,0} = 0$ \\
\bottomrule
\end{tabular}
\end{table}

The constant cross-section runs (\code{S1}--\code{S10}) bracket the
observationally allowed range for Milky Way--mass haloes
\citep{Kaplinghat2016,Sagunski2021}.  Beyond elastic scattering, we
include three classes of non-standard dark matter interactions:
dissipative SIDM (\code{dSIDM}), in which a fraction $f_{\rm diss}$ of
the centre-of-mass kinetic energy is lost per scatter
\citep{Shen2021}; DM--baryon momentum exchange (\code{IDM}), where a
continuous drag force couples the dark matter and baryonic fluids; and
two-state inelastic scattering (\code{iSIDM}), with an exothermic
variant ($f_{\rm exc,0} = 0.3$) and an endothermic variant ($f_{\rm
  exc,0} = 0$) \citep{Schutz2015,Vogelsberger2019}.
