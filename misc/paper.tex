...
cosmological simulation code that significantly reduce both memory
consumption and communication overhead. The central innovation is a
\ksec\ domain decomposition that replaces the traditional Hilbert
curve ordering with a recursive $k$-ary spatial partitioning, enabling
hierarchical MPI communication with $\Olog{\sum_l k_l}$ messages per
exchange instead of the $\Olog{\ncpu^2}$ scaling of the original
\code{MPI\_ALLTOALL} pattern. We further eliminate the large
neighbour-pointer array by introducing a Morton key hash table for
$\Olog{1}$ neighbour lookup, saving $48 \times \ngridmax$ bytes per
rank. Combined with on-demand allocation strategies for auxiliary
arrays, the total steady-state memory reduction exceeds 1\,GB per
rank for typical production configurations. Additional optimizations
to the multigrid Poisson solver --- including precomputed neighbour
caches, merged red-black exchanges, and fused residual-norm
computation --- reduce the Poisson fraction of total runtime from
55\,per\,cent to 39\,per\,cent. A spatial hash binning scheme for
Type~II supernova feedback achieves two orders of magnitude speedup
over the original brute-force algorithm. We also implement
...
bit-level physical consistency, as verified by conservation-law
diagnostics across extensive test suites.
\end{abstract}
...
\begin{itemize}
\item A \ksec\ domain decomposition (Section~\ref{sec:ksection}) that
  replaces Hilbert ordering with a recursive multi-way spatial
...
\item A spatial hash binning scheme for Type~II supernova feedback
  (Section~\ref{sec:feedback}), accelerating the SN averaging and
  blast injection by factors of 160--260$\times$.
...
% ===================================================================
\section{K-Section Domain Decomposition}
\label{sec:ksection}
...
Our \ksec\ decomposition replaces the one-dimensional Hilbert
...
% ===================================================================
\section{SNII Feedback Spatial Binning}
\label{sec:feedback}
...
All tests use a cosmological simulation with $200 \times 10^6$
dark matter particles in a periodic box, evolved for 10 coarse time
steps from a GRAFIC2 initial condition generated with \textsc{music}
\citep{Hahn2011}. The AMR hierarchy extends to a maximum of
$\nlevelmax = 12$ refinement levels. Tests are run on a shared-memory
node with 64 CPU cores, using 12 MPI ranks to leave resources for
concurrent jobs.
...
\emph{[Placeholder: This section will be filled with detailed timing
breakdowns from production-scale runs. Planned content includes:
(i) strong scaling tests from 12 to 512 ranks, (ii) communication
time as a fraction of total for Hilbert vs.\ k-section ordering,
(iii) memory high-water mark comparison, and (iv) MG solver timing
breakdown before and after optimizations.]}

\begin{figure}
\centering
% \includegraphics[width=\columnwidth]{scaling_placeholder.pdf}
\caption{[Placeholder] Strong scaling comparison between standard
\ramses\ (Hilbert ordering) and \curamses\ (\ksec\ ordering) for a
cosmological simulation with $200 \times 10^6$ particles. The
\ksec\ version shows improved scaling beyond 128 ranks due to the
elimination of \code{MPI\_ALLTOALL} communication.}
\label{fig:scaling}
\end{figure}

\begin{figure}
\centering
% \includegraphics[width=\columnwidth]{memory_placeholder.pdf}
\caption{[Placeholder] Peak memory per rank as a function of the
number of MPI ranks. The \curamses\ optimizations (Morton hash,
Hilbert key elimination, on-demand arrays) reduce the per-rank
memory by over 1\,GB compared to standard \ramses.}
\label{fig:memory}
\end{figure}

\begin{figure}
\centering
% \includegraphics[width=\columnwidth]{mg_timing_placeholder.pdf}
\caption{[Placeholder] Timing breakdown of the multigrid Poisson
solver per coarse step, comparing the standard implementation with
the optimized version. The precomputed neighbour cache, merged
red-black exchange, and fused residual-norm reduce the MG fraction
from 55\,per\,cent to 39\,per\,cent of total runtime.}
\label{fig:mg_timing}
\end{figure}

\begin{figure}
\centering
% \includegraphics[width=\columnwidth]{feedback_placeholder.pdf}
\caption{[Placeholder] Wall-clock time for the SNII feedback routines
(\code{average\_SN} and \code{Sedov\_blast}) as a function of the
number of simultaneous SN events. The spatial binning algorithm scales
linearly with $N_{\rm SN}$, while the brute-force algorithm scales
quadratically.}
\label{fig:feedback}
\end{figure}
...
\emph{[Placeholder: Comparison of peak-to-mean memory ratios and
maximum particle counts per rank with and without memory-weighted
balancing. Tests show a reduction in peak-to-mean memory ratio from
$\sim$2.5 to $\sim$1.3 with memory-weighted balancing enabled.]}
...
\begin{enumerate}
\item \textbf{K-section domain decomposition.} A recursive $k$-ary
...
\item \textbf{SNII feedback spatial binning.} A spatial hash binning
  scheme reduces the feedback computation from
  $\Olog{N_{\rm cells} \times N_{\rm SN}}$ to
  $\Olog{N_{\rm cells} \times 27\,\bar{n}_{\rm SN/bin}}$, achieving
  160--260$\times$ speedups with $\sim$2000 simultaneous events.
