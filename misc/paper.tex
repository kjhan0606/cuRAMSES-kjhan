...
A Morton key (also known as a Z-order key) is a 128-bit integer formed
...
\end{equation}
where $B = 42$ bits per coordinate and $\text{bit}_b(n)$
...
2^{l-1})$. With $B = 42$ bits the maximum representable coordinate is
$2^{42} - 1 \approx 4.4 \times 10^{12}$, which accommodates up to level 43 for
$n_x = 1$ or level 41 for $n_x = 4$. The encoding and decoding
...
\begin{align}
t   &= \bigl[(M \times \phi_1) \oplus (M \gg 16)\bigr] \times \phi_2,
\nonumber \\
h(M) &= t \oplus (t \gg 13),
\label{eq:hash_func}
\end{align}
...
\caption{Schematic of the Morton key hash table that replaces the
  ...
  $(i_x, i_y, i_z)$ are bit-interleaved into a 128-bit Morton key $M$,
...
$B = 42$ bits is computed by the following bit-manipulation loop:
