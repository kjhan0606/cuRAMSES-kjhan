...
A Morton key (also known as a Z-order key) is an integer formed
...
We apply several targeted optimizations to the MG smoother and
...
The red and black Gauss--Seidel sweeps are merged by removing the
...
decomposition along the $x$-axis.  Each rank owns $\lceil N_x /
\ncpu \rceil$ contiguous $x$-slabs.  Data redistribution between
...
grid index range.  The same test is evaluated on both sides (sender
...
\mathrm{INT}(L_{\rm box} \,/\, r_{\rm max})\bigr)\!\bigr)$
...
The \emph{production test}, used for strong scaling
