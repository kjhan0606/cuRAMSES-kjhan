...
\citep{HernandezAguayo2023}, and the cosmic web morphology
\citep{Angulo2012}, with percent-level precision requires simulation
...
ranks (Algorithm~\ref{alg:exclusive}).  At each level~$l$, a node is
...
\begin{algorithm}[H]
\caption{Hierarchical \ksec\ exchange}
\label{alg:exclusive}
\begin{algorithmic}[1]
...
infrastructure.
Working buffers use Fortran 2003 \code{move\_alloc} for
zero-copy swaps, and per-level arrays are pre-allocated with
\code{save} attributes and grow-only semantics to eliminate
allocation overhead on the 100+ exchange calls per time step.
...
\midrule
Message count & $\Olog{N_{\rm nbr}}$ & $\Olog{\ncpu}$ & $\Olog{\sum_l k_l}$ \\
Buffer memory & $\Olog{N_{\rm nbr} \cdot N_{\rm ghost}}$ &
  $\Olog{\ncpu \cdot N_{\rm ghost}}$ &
  $\Olog{k_{\rm max} \cdot N_{\rm ghost}}$ \\
Synchronisation & none & implicit barrier & none \\
\bottomrule
\end{tabular}
\end{table}
...
\item[Phase~2 (running)]  The selected backend is used for production
  ...
  \label{eq:ema}
  \end{equation}
  where $t_n$ is the elapsed time of the current call.  After
...
\bibitem[\protect\citeauthoryear{Angulo et al.}{Angulo et al.}{2012}]{Angulo2012}
...
\bibitem[\protect\citeauthoryear{Hern{\'a}ndez-Aguayo et al.}{Hern{\'a}ndez-Aguayo et al.}{2023}]{HernandezAguayo2023}
Hern{\'a}ndez-Aguayo C., et al., 2023, MNRAS, 524, 2556
