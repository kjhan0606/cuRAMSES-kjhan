...
densest regions of the simulation: the cores of dark matter haloes, the
circum-galactic medium, and the sites of active star formation.  In
practice, this means that phenomena requiring high spatial resolution
--- e.g., the fragmentation of cold gas clouds in the ISM, the
accretion flow onto supermassive black holes, or the propagation of
supernova-driven outflows --- can be resolved to higher AMR levels
...
\paragraph{Small grid ($N \le 256^3$).}
...
\paragraph{Sparse point-to-point exchange.}
...
optimisation (Section~\ref{sec:multigrid}) is more effective than GPU
...
We note that the strong scaling tests above are performed on a single
shared-memory node.  Multi-node scaling introduces additional
inter-node communication overhead (network latency, bandwidth
contention) that is not captured by these intra-node measurements.
Microbenchmark results on a 16-node cluster (Section~\ref{sec:bench_ghost})
show that the \ksec\ domain decomposition halves the ghost-list build
time compared to Hilbert ordering and maintains comparable exchange
latency via sparse P2P communication; full-application multi-node
scaling tests are planned for a forthcoming study.
...
(Table~\ref{tab:conservation}).  The only measurable difference is a
