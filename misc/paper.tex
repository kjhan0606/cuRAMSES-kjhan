...
number of communication partners (6) regardless of the total rank
...
$N_{\rm nb}$: number of neighbour ranks;
$N_{\rm gh}$: ghost grids per rank;
$k_l$: branching factor at tree level $l$;
$k_{\rm max} = \max(k_l)$.}
...
\item[Phase~2 (running)]  The selected backend is used for production
  ...
  \label{eq:ema}
  \end{equation}
  where $t_n$ is the elapsed time of the current call and
  $\bar{t}_n$ is the running EMA from the previous step.  After
...
Here $n_x$ denotes the number of
...
respectively.
All Morton key routines use a parametric kind parameter
\code{mkb} (8 or 16 bytes), so the same source code handles both
widths without modification.
...
computes which slabs to reach; receiver computes which domains
...
linked particles are swapped to the front of the working array and the
...
\caption{FoF merger wall-clock time as a function of the number of
  ...
  as $\Olog{N\log N}$; the MPI\,+\,OpenMP oct-tree (green; 3~MPI
...
``Tree'' is the serial oct-tree; ``OMP'' is the MPI\,+\,OpenMP tree
...
\caption{Ghost zone exchange benchmark for $N=1024$ ($10^9$~cells,
  ...
  rank counts.
  Build: one-time ghost list construction;
  Exch: per-exchange latency averaged over 3~repeats.
  SV: surface-to-volume ratio; Nbr: average neighbour CPU count.}
