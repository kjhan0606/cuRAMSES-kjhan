...
The multigrid (MG) Poisson solver \citep{Briggs2000,Trottenberg2001} consumes a large fraction of the
...
The standard multigrid V-cycle solves the Poisson equation by
recursively transferring residuals between AMR levels.  Starting at
the finest level, the algorithm \emph{smooths} the current
approximation using a red-black Gauss--Seidel (GS) iteration,
computes the residual, and \emph{restricts} it to the next coarser
level.  This restriction--smooth sequence descends to the coarsest
level, where a direct solve (or additional smoothing) is applied.
The correction is then \emph{prolongated} (interpolated) back up
through each level, with post-smoothing at each step to damp
high-frequency errors introduced by the interpolation.
Figure~\ref{fig:mg_vcycle} illustrates the V-cycle structure and
the exchange optimizations at each level.

Each smoothing step requires ghost-zone exchanges between MPI ranks.
We apply several targeted optimizations to the smoother and
communication pattern.  Neighbour grid indices are precomputed into a
contiguous cache before the V-cycle, amortising hash table lookups.
The red and black GS sweeps are merged by removing the
inter-sweep ghost exchange, and two redundant residual exchanges are
eliminated, reducing the total exchange count from 9 to 5 per
iteration (44\,per\,cent).  Implementation details are given in
...
\caption{Schematic of the multigrid V-cycle for four AMR levels.
  The left (descending) leg restricts the residual from fine to
  coarse; the right (ascending) leg prolongates the correction.  At
  each level, red-black Gauss--Seidel smoothing is applied.  The
  optimized exchange reduces the number of ghost-zone communications
  from 9 to 5 per iteration by merging the red and black sweeps and
  eliminating redundant residual exchanges.}
\label{fig:mg_vcycle}
\end{figure}
