...
\begin{figure}
\centering
\includegraphics[width=\columnwidth]{../docs/manual/ksection_progressive.png}
\caption{Progressive \ksec\ decomposition for $\ncpu = 12 = 3 \times
2 \times 2$. Left: the root domain is split into $k = 3$ slabs along
the longest axis. Centre: each slab is bisected ($k = 3 \times 2 = 6$
sub-domains). Right: each half is bisected again ($k = 3 \times 2
\times 2 = 12$ leaf domains). Colours distinguish individual MPI
ranks; particles are shown as dots to indicate the load distribution.}
