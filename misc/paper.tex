...
Poisson solver accounts for approximately 55\,per\,cent of the total
wall-clock time per coarse step.  We describe two complementary
...
fraction to approximately 39\,per\,cent.
...
(prolongation).  At each AMR level the smoother is a \emph{red-black
...
pre-smoothing, computes the residual $r = f - A\phi$, and
\emph{restricts} the residual to the next coarser level.  This
...
\item \textbf{Precomputed neighbour cache.}  Neighbour grid indices
  ...
  amortising the cost of hash table lookups over all iterations
  (see Appendix~\ref{app:mg_details}).
\item \textbf{Merged red-black sweep.}  The inter-sweep ghost
  exchange between the red and black passes is removed: both colours
  are updated using locally available data, relying on the fact that
  the one-step-stale boundary values introduce only an $O(h^2)$
  perturbation that is corrected by subsequent iterations.
\item \textbf{Fused residual and norm.}  The residual $r$ and its
  $L^2$ norm $\|r\|_2^2$ are computed in a single pass, eliminating
  a redundant exchange.
\end{enumerate}
These reduce the exchange count from 9 to 5 per iteration
($-44$\,per\,cent), as shown in the lower panel of
Figure~\ref{fig:mg_vcycle}.  Implementation details are given in
...
slab ranges using a symmetric overlap test,
\begin{equation}
  \text{overlap}(A, B) \;\equiv\;
  x_{\max}^{(A)} > \frac{s_{\min}^{(B)} - 0.5}{N_x}
  \;\wedge\;
  x_{\min}^{(A)} < \frac{s_{\max}^{(B)} + 1.5}{N_x}
\label{eq:fftw_overlap}
\end{equation}
where $[x_{\min}^{(A)}, x_{\max}^{(A)}]$ is rank~$A$'s spatial
bounding box and $[s_{\min}^{(B)}, s_{\max}^{(B)}]$ is slab~$B$'s
grid index range.  The asymmetric padding ($-0.5$ and $+1.5$) arises
from cell-centred positioning: a cell at integer index~$s$ spans
$[s - 0.5,\, s + 0.5]$ in grid coordinates, and adding one cell of
ghost padding on the right extends the upper bound by an additional
$+1$.  The same test is evaluated on both sides (sender
computes which slabs to reach and the receiver computes which domains
overlap), guaranteeing that every \code{MPI\_ISEND} has a matching
\code{MPI\_IRECV}.  Periodic boundary wrapping is handled by
extending the overlap check when $s_{\min} < 0$ or $s_{\max} \ge
N_x$.  Partner lists are cached and invalidated only when the domain
...
without requiring a GPU.  Energy conservation is identical,
$e_{\text{cons}} = 6.12 \times 10^{-4}$ for both FFTW3 and cuFFT.
...
An oct-tree is built over the sink positions.  For each seed particle,
...
(\code{EraseFromTree}) so subsequent searches skip them.  This is
equivalent to the brute-force result but dramatically faster for
$N_{\rm sink} \gtrsim 500$.

\paragraph{MPI\,+\,OpenMP oct-tree.}
The particle index range is divided among MPI ranks, and each rank's
sub-range is further divided among OpenMP threads.  All threads share
the same read-only tree but maintain private inclusion and group-ID
arrays.  A thread-safe variant of \code{FoF\_link}
(\code{Omp\_FoF\_link}) replaces \code{EraseFromTree} with a
thread-local \code{included} array, enabling concurrent traversal
without locks.
...
of preceding threads, and MPI ranks chain their offsets via sequential
\code{MPI\_Send}/\code{MPI\_Recv}.  The final merged result is broadcast
to all ranks.
...
\caption{FoF merger wall-clock time as a function of the number of
  ...
  as $\Olog{N\log N}$, and the MPI\,+\,OpenMP oct-tree (green, 3~MPI
  $\times$ 4~OMP threads) achieves a further ${\sim}4.3\times$
  parallel speedup.  At $N_{\rm sink}=10^5$, the parallel tree is
...
``Tree'' is the serial oct-tree, and ``OMP'' is the MPI\,+\,OpenMP tree
(3~MPI $\times$ 4~threads).}
\label{tab:fof_perf}
\footnotesize
