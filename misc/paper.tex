...
spatial partitioning.  This approach eliminates all global
\code{MPI\_ALLTOALL} calls and dramatically reduces the message
complexity per ghost-zone exchange, demonstrating highly efficient
strong scaling. 
...
allocation, successfully reducing the memory footprint by over 1\,GB
per MPI rank.  Furthermore, a novel spatial hash binning algorithm
...
model utilizing GPU-resident mesh data and halo-only PCIe transfers.
...
properties of the large-scale structure --- the halo mass function, the
...
galaxies --- supermassive black hole (SMBH) feedback, the multi-phase
...
advantage---reducing the number of communication partners from
...
Cross-boundary groups---those whose transitive closure extends into a
...
restriction --- eliminating per-iteration PCIe transfers entirely.
...
scope --- replacing the octree data structure, restructuring all MPI
...
the \ksec\ domain decomposition, Morton key hash table, memory
...
\caption{Ghost zone exchange benchmark for $N=1024$ ($10^9$~cells,
  $n_{\rm var}=5$) on the \textit{Grammar} cluster (64--1024 MPI ranks).
...
--- \texttt{MPI\_ALLREDUCE}, a manual binary-tree allreduce (reduce to
...
red-black Gauss-Seidel, and unpreconditioned conjugate gradient (CG) ---
...
management, all conservation diagnostics --- total energy
