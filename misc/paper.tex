...
rigorously preserve physical consistency: mass, momentum, and energy
...
properties of the large-scale structures, such as the halo mass function, the
galaxy luminosity function, and the cosmic web morphology, with
percent-level precision requires simulation volumes exceeding
$(1\,h^{-1}\,\mathrm{Gpc})^3$ with $10^{11}$ or more particles.
At the same time, resolving the physics that governs individual
galaxies, including supermassive black hole (SMBH) feedback, the multi-phase
interstellar medium (ISM), and star formation on sub-parsec scales,
demands deep AMR hierarchies with dynamic ranges of $10^5$ or more.
...
well into the exascale regime.  The principal barriers to reaching this
regime are not physical modelling limitations but rather
\emph{algorithmic}: communication overhead that grows with the number
of MPI ranks, memory consumption that limits the refinement depth per
node, and solver inefficiencies that dominate the wallclock time.
...
infeasible: the AMR tree structure, including parent--child links, neighbour
...
\item[Phase~1 (\ksec\ trial)]  The next $N_{\rm trial}$ calls use
  ...
  initial decision: the backend with the smaller global mean is
  selected, and the per-call average is used to initialise the
...
densest regions of the simulation: the cores of dark matter haloes,
...
with $\Olog{N_{\rm sink}^2}$ brute-force pair enumeration: for each
...
lower thread's index range, are \emph{rolled back}: the partial group
...
consolidated: thread-local group IDs are offset by the cumulative count
...
at every decimal place: $e_{\rm cons} = 3.77 \times 10^{-3}$,
$e_{\rm pot} = -1.88 \times 10^{-6}$, $e_{\rm kin} = 1.23 \times
