...
A hybrid CPU/GPU dispatch model allows OMP threads to dynamically
offload compute-intensive routines --- including the Godunov solver,
gravity force computation, and radiative cooling --- to GPU streams at
runtime, with automatic fallback to CPU execution when streams are
unavailable. We also implement variable-$\ncpu$ restart for both HDF5 and
native binary checkpoint formats, removing the constraint that
checkpoint files must be read with the same number of MPI ranks as
...
standard \ramses\ implementation, ghost-zone exchange requires each
rank to broadcast its emission and reception lists to all other ranks
via \code{MPI\_ALLTOALL}, leading to $\Olog{\ncpu^2}$ message
...
rank, so restarting with a different rank count requires a cumbersome
intermediate redistribution step.
...
saving $\sim$240\,MB per rank, and on-demand allocation of redundant
large arrays (Section~\ref{sec:memory}) yields over 1\,GB of
additional savings. Algorithmic optimizations to the multigrid Poisson
...
AGN feedback by orders of magnitude. A hybrid CPU/GPU dispatch model
(Section~\ref{sec:hybrid}) dynamically offloads compute-intensive
routines to GPU streams at runtime. We also implement variable-$\ncpu$
restart for both HDF5 and binary formats (Section~\ref{sec:varcpu}), along with
miscellaneous improvements described in Section~\ref{sec:misc}.
...
isotropic sub-domains, minimising the surface-to-volume ratio and
...
Fig.~\ref{fig:scaling} visualises these trends.  Panel~(a) shows the
per-component timer values as a function of $\ncpu$.  Particles,
Poisson, and flag operations decrease monotonically, following the
ideal scaling line at low rank counts before flattening due to
communication overhead.  The MG solver plateaus near 5.5\,s beyond
32~ranks, limited by inter-rank ghost-zone communication in the
V-cycle.  The Godunov solver increases at high $\ncpu$ because
fine-level AMR patches become localised and load-imbalanced.
Panel~(b) shows the speedup relative to 4~ranks: Particles and Flag
achieve $9.4\times$ and $8.0\times$ speedup at 64~ranks, respectively,
while the total elapsed time peaks at $2.2\times$ speedup (16~ranks)
before communication costs dominate.

\begin{figure*}
\centering
\includegraphics[width=\textwidth]{strong_scaling.pdf}
\caption{Strong scaling of \curamses\ on a dual-socket AMD EPYC 7543
node (64 cores) with a $200 \times 10^6$ particle cosmological
simulation.  (a)~Per-component wall-clock times versus $\ncpu$; the
dashed line shows ideal scaling from the 4-rank baseline.
(b)~Speedup relative to 4~ranks.  Particles and flag operations scale
nearly ideally, while the MG solver and Godunov hydro are limited by
communication overhead and load imbalance at high rank counts.}
\label{fig:scaling}
\end{figure*}
...
\item \textbf{Variable-ncpu restart.} Both HDF5 parallel I/O and
  distributed binary I/O enable checkpoint/restart with arbitrary rank
...
one-dimensional space-filling curve ordering. The hybrid CPU/GPU
dispatch model demonstrates that GPU acceleration can be integrated
into a legacy Fortran codebase without sacrificing portability: the
same source compiles and runs correctly in CPU-only mode when CUDA is
unavailable, making it practical for heterogeneous computing
environments where not all nodes are equipped with GPUs.

Looking ahead, the three-level MPI+OpenMP+CUDA parallelism
positions \curamses\ well for exascale platforms such as Frontier,
Aurora, and LUMI, where the majority of floating-point throughput
resides in GPU accelerators. Further optimisation of the host-to-device
data transfer --- currently the dominant cost in the GPU pipeline ---
through persistent device-side data structures and asynchronous
prefetching will be a key focus of future work.
