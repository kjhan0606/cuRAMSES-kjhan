...
A hybrid CPU/GPU dispatch model
...
the domain hierarchy and the communication pattern. At each level $l$, the
...
\begin{equation}
C_{\rm cell} = \twotondim^{-1}\bigl(w_{\rm grid} +
...
The overlap exchange handles items with spatial extent (bounding
boxes) that may overlap multiple sub-domains. At each tree level, an
item is replicated to all children whose bounding box intersects the
item's spatial extent. This is used for operations such as the
supernova and AGN feedback, where each blast has a finite physical
radius.
...
back into the emission grids. An integer variant converts integer
data to double precision for transport and recovers the integer
values on receipt. Finally, dedicated multigrid variants handle the
solver's active and emission grid structures with the same
...
\end{equation}
For $\ngridmax = 5\,\mathrm{M}$, this is 240\,MB per rank. Moreover,
