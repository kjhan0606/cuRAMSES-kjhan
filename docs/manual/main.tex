...
\end{enumerate}
The split positions at each level are determined by the load-balancing histogram to ensure equal computational cost per subdomain.  The color hue in Figure~\ref{fig:ksection_progressive} encodes the $x$-slab membership (red/green/blue), with saturation and brightness variations indicating the $y$ and $z$ subdivisions, respectively.

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{ksection_progressive.png}
\caption{Progressive k-section decomposition for $k_1 \times k_2 \times k_3 = 3 \times 2 \times 2 = 12$ CPUs.  \textbf{Left}: first split into 3 slabs along $x$.  \textbf{Center}: each slab bisected along $y$ ($3 \times 2 = 6$ subdomains).  \textbf{Right}: final bisection along $z$ ($3 \times 2 \times 2 = 12$ subdomains).  Each face shows projected column density $\log_{10}(1+\Sigma)$ from a $4096^3$ TSC density field.}
...
The GPU acceleration is enabled at build time with \texttt{make HDF5=1 USE\_CUDA=1}.  At runtime, each MPI rank initializes a pool of 8 CUDA streams, distributed across available GPUs by local rank (\texttt{local\_rank \% device\_count}).  The multi-architecture binary supports sm\_80 (A100), sm\_86 (A10), and sm\_90 (H100) targets simultaneously.
...
The host-side superbatch buffers are allocated per-thread via OMP \texttt{private} clauses and passed to external flush subroutines as explicit-shape arrays to avoid Intel ifx compiler issues with \texttt{c\_loc} on assumed-shape arrays.
...
\end{lstlisting}
This enables a single binary to run on A100 (sm\_80), A10 (sm\_86), and H100 (sm\_90) GPUs without recompilation.

\section{Verification}

The GPU-accelerated code produces bit-identical physics results compared to the CPU-only build:
\begin{itemize}[nosep]
\item 4 ranks $\times$ 1 GPU (A10): $e_{\rm cons} = 5.23 \times 10^{-3}$, $e_{\rm pot} = -1.88 \times 10^{-6}$, $e_{\rm kin} = 1.23 \times 10^{-6}$ at step~10
\item 4 ranks $\times$ 4 GPUs (A100): identical physics results with correct per-rank GPU assignment
\end{itemize}
