...
\texttt{read\_params.jaehyun.f90} & Define and read \texttt{\&COSMO\_PARAMS} namelist; 3-way DE validation \\
...
Neutrino parameters are split across two namelist blocks:
...
\texttt{read\_params.jaehyun.f90} & Read neutrino parameters from namelist \\
...
\texttt{cs2\_de} & real(dp) & 0 & DE sound speed squared $c_s^2$ (see Sec.~\ref{sec:de_perturb}) \\
\midrule
...
\midrule
\multicolumn{4}{l}{\textit{Dark energy perturbation (new)}} \\
\midrule
\texttt{de\_perturb} & logical & \texttt{.false.} & Enable DE density perturbation in Poisson solver (Sec.~\ref{sec:de_perturb}) \\
\texttt{de\_table} & char(256) & \texttt{''} & Path to CAMB DE transfer function table ($R_{\mathrm{DE}}(k,a)$) \\
\midrule
\multicolumn{4}{l}{\textit{Massive neutrino (new)}} \\
\midrule
...
\begin{itemize}[nosep]
  ...
  \item \texttt{de\_perturb}: with \texttt{de\_table} $\to$ table-based (any $c_s^2$); without $\to$ requires $c_s^2 > 0$.
