...
\textbf{Requires:}
\begin{itemize}[nosep]
  \item \paramref{omega_nu} $> 0$ in \texttt{\&COSMO\_PARAMS}
...
IC file header and for specifying CPL dark energy equation-of-state
parameters.  When this block is absent, default values are used
($w_0=-1$, $w_a=0$, which recovers the standard $\Lambda$CDM model).
...
in conjunction with \paramref{de_perturb}\texttt{=.true.}\ to include
dark energy perturbations in the FFT Poisson solver.  Set to~0 for the
standard smooth dark energy (cosmological constant) assumption.

When $c_s^2 > 0$ and DE perturbations are enabled, the FFT solver
applies a $k$-dependent correction factor:
\[
  \hat\phi(\mathbf{k}) \;\leftarrow\; \hat\phi(\mathbf{k}) \times
  \frac{k^2 + \kappa^2}{k^2 + \kappa^2 + \alpha}
\]
where $\kappa^2$ and $\alpha$ are computed from $c_s^2$, $w(a)$, and
the background DE density at the current scale factor.

Typical values: $c_s^2 = 1$ (speed-of-light sound speed, most common
in literature) or $c_s^2 \ll 1$ for clustering dark energy models.

\apinote{Only effective in the FFT solver path (\paramref{use_fftw} or
\paramref{gpu_fft}).  The multigrid V-cycle on AMR levels does not
include DE perturbation corrections.}
\end{quote}
...
\begin{quote}
Enable dark energy density perturbations in the Poisson solver.  When
\texttt{.true.}\ and \paramref{cs2_de}~$> 0$, the FFT solver modifies
the gravitational potential in Fourier space to account for the
clustering of dark energy.

\textbf{Requires:}
\begin{itemize}[nosep]
  \item \paramref{cosmo}\texttt{=.true.}
  \item \paramref{cs2_de}~$> 0$
  \item FFT solver active (\paramref{use_fftw} or \paramref{gpu_fft})
\end{itemize}

If \texttt{cs2\_de}~$\leq 0$ or the run is not cosmological, the code
prints a warning and disables this flag.
...
\texttt{use\_neutrino}
  ...
  & All three must be set for neutrino response \\
\addlinespace
\texttt{de\_perturb}
  & \texttt{cs2\_de}, \texttt{w0}, \texttt{wa}
  & DE perturbation requires $c_s^2 > 0$ and CPL EOS \\
\addlinespace
