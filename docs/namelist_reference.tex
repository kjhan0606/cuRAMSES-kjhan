...
table-based linear response method.  The table contains
$R_{\mathrm{DE}}(k, a) = \delta_{\mathrm{DE}} / \delta_m$ on a
$(N_k \times N_a)$ grid, in the same ASCII format as
\paramref{neutrino_table}.
...
python3 misc/generate_de_table.py --w0=-0.9 --wa=0.1 --cs2=0.0 \
    -o de_table.dat
\end{verbatim}
The script uses CAMB's \texttt{get\_redshift\_evolution()} to compute
$\delta_{\mathrm{DE}}(k,z)$ and $\delta_m(k,z)$ directly, supporting
arbitrary $c_s^2$ including $c_s^2 = 0$.  Default grid: 200~$k$-points
$\times$ 50~$a$-points (same as the neutrino table).
...
scattering.  Requires \paramref{sidm}\texttt{=.true.}\ and
...
\begin{quote}
DM self-interaction cross-section per unit mass $\sigma/m$ in
cm$^2$\,g$^{-1}$.  Controls the scattering probability for each
particle pair within a cell:
\[
  P = \frac{\sigma}{m} \times m_p \times v_{\mathrm{rel}} \times
  \frac{\Delta t}{V_{\mathrm{cell}}} \times (N - 1)
\]
where $m_p$ is the DM particle mass, $v_{\mathrm{rel}}$ is the
relative velocity of the pair, $\Delta t$ is the time step,
$V_{\mathrm{cell}}$ is the cell volume, and $N$ is the number of DM
particles in the cell.

Typical observational constraints allow $\sigma/m \sim 0.1$--$10\;
\mathrm{cm}^2\,\mathrm{g}^{-1}$, with values $\gtrsim 1$ producing
significant core formation in dwarf galaxy haloes.

\apinote{Upon scattering, the two particles' velocities are isotropically
randomised in their centre-of-mass frame, conserving total momentum
and kinetic energy (elastic scattering).}
\end{quote}
...
Cells with fewer than \texttt{sidm\_npart\_min} dark matter particles
are skipped entirely.  The default value of~2 is the physical minimum
(a pair is needed for scattering).

Increasing this value can reduce noise in poorly sampled regions at the
cost of suppressing scattering in low-density cells.
\end{quote}

\paragraph{Diagnostic output.}
When scattering events occur, the code prints per-level statistics
including the number of scattered pairs, cumulative momentum change
$\Delta\mathbf{p}$ (g\,cm/s), cumulative kinetic energy change
$\Delta E_k$ (erg), and the maximum single-event values.
These quantities should be at machine-precision level
($\sim 10^{-15}$ relative to particle momentum), confirming
exact conservation up to floating-point roundoff.
