program part2birth
  ...
  real(KIND=8)::age
  real(kind=8),dimension(:),allocatable :: birth_date
  ...
  do k=1,ncpu_read
     ...
     nomfich=TRIM(repository)//'/birth_form'//TRIM(nchar)//'.out'//TRIM(ncharcpu)
     open(unit=1,file=nomfich,status='new',form='formatted')     
     do i=1,npart2
        if(id(i)> 0.and.birth(i)/=0) write(1,*) birth_date(i), m(i)
     enddo
     close(1)
     deallocate(x,m)
     deallocate(birth,id)
