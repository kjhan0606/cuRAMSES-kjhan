...
!!$omp parallel do private(ind,iskip,i)
  ...
  if(balance)then
!!$omp parallel do private(ind,iskip,i) reduction(+:iflag)
     do ind=1,twotondim
        iskip=ncoarse+(ind-1)*ngridmax
        do i=1,active(ilevel)%ngrid
...
!!$omp parallel do private(ind,iskip,i) reduction(+:iflag)
        do ind=1,twotondim
           ...
           iflag=iflag+active(ilevel)%ngrid
        end do
        nflag = nflag + iflag
     end if
  end if
...
subroutine test_flag(ilevel)
  ...
  ! Loop over cells
!!$omp parallel do private(ind,iskip,i,ind_grid_son,ok,ind_son,iskip_son,ind_cell_son) reduction(+:iflag)
  do ind=1,twotondim
     iskip=ncoarse+(ind-1)*ngridmax
...
!################################################################
!################################################################
subroutine sub_ensure_ref_rules(ilevel,igrid,ngrid)
  use amr_commons
  implicit none
  integer::ilevel
  !-----------------------------------------------------------------
...
end subroutine sub_ensure_ref_rules
!################################################################
!################################################################
!################################################################
!################################################################
...
!!$omp parallel do private(igrid,ngrid)
  do igrid=1,ncache,nvector
	 ...
	 call sub_ensure_ref_rules(ilevel,igrid,ngrid)
  end do
...
subroutine sub_userflag_fine(ilevel,skip_loc,scale, igrid,ngrid,iflag)
  ...
  use cooling_module
  implicit none
  integer:: ilevel, igrid,ngrid,iflag
...
!jhshin1
!        ok(i)=ok(i).and.(r < 1.0)
        ok(i)=ok(i).and.(xn<1.and.yn<1.and.zn<1)
!jhshin2
     end do
  endif
...
!##########################################################
subroutine sub2_smooth_fine(ilevel, igrid,ngrid,n_nbor,ismooth)
  use amr_commons
  implicit none
  integer::ilevel
  ! -------------------------------------------------------------------
...
!##########################################################
!##########################################################
subroutine sub3_smooth_fine(ilevel, igrid,ngrid,iflag)
  use amr_commons
  implicit none
...
end subroutine sub3_smooth_fine
...
!!$omp parallel do private(igrid,ngrid)
     do igrid=1,ncache,nvector
        ngrid=MIN(nvector,ncache-igrid+1)
        call sub1_smooth_fine(ilevel,igrid,ngrid)
     enddo
!!!!!### This part can not be parallelized due to the racing conditions in it. -jhk
     do igrid=1,ncache,nvector
        ngrid=MIN(nvector,ncache-igrid+1)
        call sub2_smooth_fine(ilevel,igrid,ngrid,n_nbor,ismooth)
     enddo
!!$omp parallel do private(igrid,ngrid,jflag) reduction(+:iflag)
     do igrid=1,ncache,nvector
        ngrid=MIN(nvector,ncache-igrid+1)
        call sub3_smooth_fine(ilevel,igrid,ngrid,jflag)
        iflag = iflag + jflag
     enddo
     nflag = nflag + iflag
     ! Update boundaries
