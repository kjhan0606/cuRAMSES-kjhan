...
subroutine star_formation(ilevel)
  ...
  ! other variables
  integer ::ncache,nnew,ivar,irad,ngrid,icpu,index_star,ndebris_tot,ilun,indebris_tot,jndebris_tot
...
! get values of uold for density and velocities in virtual boundaries
  ...
  ! Loop over grids
  ncache=active(ilevel)%ngrid
!!$omp parallel do private(igrid,ngrid,indebris_tot, intot,imstar_lost,imstar_tot) reduction(+:jndebris_tot,jntot,jmstar_lost,jmstar_tot)
  do igrid=1,ncache,nvector
     ngrid=MIN(nvector,ncache-igrid+1)
	 call sub2_star_formation(ilevel, igrid,ngrid,indebris_tot,intot,imstar_lost, imstar_tot)
	 jndebris_tot = jndebris_tot + indebris_tot
	 jntot = jntot + intot
	 jmstar_lost = jmstar_lost + imstar_lost
	 jmstar_tot = jmstar_tot + imstar_tot
  end do
  ndebris_tot = ndebris_tot + jndebris_tot
  ntot = ntot + jntot
