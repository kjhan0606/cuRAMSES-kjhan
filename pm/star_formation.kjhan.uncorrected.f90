...
subroutine star_formation(ilevel)
  ...
  ! other variables
  integer ::ncache,nnew,ivar,irad,ngrid,icpu,index_star,ndebris_tot,ilun,indebris_tot
  integer ::igrid,ix,iy,iz,ind,i,j,n,iskip,istar,inew,nx_loc
  integer ::ntot,intot,ntot_all,info,nstar_corrected,ideb,ndeb,ncell
  ...
  integer:: imstar_lost, imstar_tot
...
! get values of uold for density and velocities in virtual boundaries
  ...
  ! Loop over grids
  ncache=active(ilevel)%ngrid
!!$omp parallel do private(igrid,ngrid) schedule(dynamic) reduction(+:ndebris_tot,ntot,mstar_lost,mstar_tot)
  do igrid=1,ncache,nvector
     ngrid=MIN(nvector,ncache-igrid+1)
	 call sub2_star_formation(ilevel, igrid,ngrid,indebris_tot,intot,imstar_lost, imstar_tot)
	 ndebris_tot = ndebris_tot + indebris_tot
	 ntot = ntot + intot
	 mstar_lost = mstar_lost + imstar_lost
	 mstar_tot = mstar_tot + imstar_tot
  end do
...
subroutine sub2_star_formation(ilevel, igrid,ngrid,ndebris_tot,ntot,imstar_lost,imstar_tot)
  use amr_commons
  use pm_commons
  use hydro_commons
  use poisson_commons
  ...
  use random
  implicit none
  integer:: ilevel,imstar_lost,imstar_tot
  real(dp)::t0,d0,d00,e0,mgas,mcell
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v
!  real(dp),dimension(1:twotondim,1:3)::xc
