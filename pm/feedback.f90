...
!$omp end parallel do
  ...
  if(f_w==0)then
!$omp parallel do private(j, birth_time, mejecta, zloss)
     do j=1,np
 ...
 !$omp end parallel do
  ...
  rad_factor=ERAD/ESN
!$omp parallel do private(j, tauIR, RAD_BOOST)
  do j=1,np
...
!$omp parallel do private(i,iSN,x,y,z,dxx,dyy,dzz,dr_SN, dr_cell, u,v,w)
           do i=1,ngrid
              if(ok(i))then
...
!$omp end parallel do
  ...
  ekBlast=ekBlast_all
#endif
!$omp parallel do privatge(iSN, u2, v2, w2)
  do iSN=1,nSN
...
!$omp parallel do private(i, x.,y,z,iSN,dxx,dyy,dzz,dr_SN,u,v,w)
           do i=1,ngrid
              if(ok(i))then
                 ! Get gas cell position
                 x=(xg(ind_grid(i),1)+xc(ind,1)-skip_loc(1))*scale
...
!$omp parallel do private(iSN, u,v,w)
  do iSN=1,nSN
     if(vol_gas(iSN)==0d0)then
        u=vSN(iSN,1)
        v=vSN(iSN,2)
