...
static int     g_mg_ngrid  = 0;  // current ngrid (set for diagnostics)
    ...
    double dx2, int color, int safe_mode_flag)
{
    ...
    for (int ind0 = 0; ind0 < 4; ind0++) {
        ...
        // Check scan flag (flag2 indexed with Fortran 1-based → -1 for C)
                    ...
                    const int* nbor_grid, const int* igrid, int ngrid)
{
    ...
    if (ncell != g_mg_ncell) {
        ...
        g_mg_ncell = ncell;
        printf("CUDA MG: allocated cell arrays %.1f GB (ncell=%lld)\n",
               (double)need / (1024.0*1024.0*1024.0), ncell);
    }
    ...
    // Upload cell data (phi, f(:,2), f(:,3), flag2)
    ...
    cudaMemcpy(d_mg_flag2, flag2, (size_t)ncell * sizeof(int), cudaMemcpyHostToDevice);
