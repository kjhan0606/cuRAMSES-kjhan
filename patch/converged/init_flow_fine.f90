...
subroutine init_flow_fine(ilevel)
  ...
  ! If there is an extra variable for refinement the 7 here should be 8 - this is just a kludge until dynamical nvar =nvar+nelt is implemented (Gareth Few)
!  if(nvar<7+nelt) then
!     write(*,*) 'Stopping run because nvar is not large enough to accommodate element slots'
!     call clean_stop
!  endif
     ...
     do ivar=1,nvar
        ...
        if(ok_file3)then
           ...
           if(ncache>0)then
              ...
              if(cosmo.and.ivar==iHydrogen)init_array=0.76 ! Set the initial H abundance - put here to overwrite normal element
