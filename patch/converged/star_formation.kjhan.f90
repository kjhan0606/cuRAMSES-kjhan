...
subroutine star_formation(ilevel)
  ...
  if(sf_birth_properties) then
     ...
     if((.not.file_exist)) then
        open(ilun, file=fileloc, form='unformatted')
     else
        open(ilun, file=fileloc, status="old", position="append", action="write", form='unformatted')
     endif
...
!!$omp parallel do private(igrid,ngrid,intot,imstar_lost,imstar_tot) reduction(+:jntot,jmstar_lost,jmstar_tot)
 ...
 !          do ielt=1,nelt
 !             cep(ind_part(i),ielt)=uold(ind_cell_new(i),ichem+ielt-1)
 !          enddo
           tpp(ind_part(i))=texp ! Birth epoch proper
           mp0(ind_part(i))=n*mstar!mp(ind_part(i))

!           if(sf_birth_properties) then !need to add texp here
!              if(sf_virial)then
!                 write(nvar_write_str,'(I2.2)') nelt+13
!                 starlog_format= '(2I10,'//trim(nvar_write_str)//'E24.16)'
!                 write(ilun,starlog_format) index_star,ilevel,birth_epoch,texp,n*mstar,x,y,z,u,v,w,d,tg,zg,uold(ind_cell_new(i),ivirial),uold(ind_cell_new(i),ichem:ichem+nelt-1) 
!              else
!                 write(nvar_write_str,'(I2.2)') nelt+12
!                 starlog_format= '(2I10,'//trim(nvar_write_str)//'E24.16)'
!                 write(ilun,starlog_format) index_star,ilevel,birth_epoch,texp,n*mstar,x,y,z,u,v,w,d,tg,zg,cep(ind_part(i),:)
!                 write(ilun,starlog_format) index_star,ilevel,birth_epoch,texp,n*mstar,x,y,z,u,v,w,d,tg,zg,uold(ind_cell_new(i),ichem:ichem+nelt-1)
!              endif
!           endif
           if(sf_birth_properties) then !need to add texp here
              if(sf_virial)then
