...
!$omp end parallel
  ...
  if(pic) then
     ...
     if(numbl(myid,ilevel)>0)then
        ...
        iekin_loc = 0
        idt_loc = dt_all
!$omp parallel private(subnump,igrid) reduction(+:iekin_loc), reduction(MIN: idt_loc)
        subnump = nparticles(mythread)
        igrid = ptrhead(mythread)
	    call sub_newdt_fine(ilevel, igrid,subnump, jdt_loc, jekin_loc)
...
!#####################################################################
!#####################################################################
subroutine sub_newdt_fine(ilevel, igrid, subnump, dt_loc, ekin_loc)
  use pm_commons
  use amr_commons
  use hydro_commons
  use poisson_commons, ONLY: gravity_type
