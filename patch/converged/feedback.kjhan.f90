...
! common /thermal_feedback_units/ scale_l,scale_t,scale_d,scale_v,scale_nH,scale_T2
  ...
  !!NEXT THING TO DO IS MOVE UNIT CONVERSION FROM XX TO TABLE AND ASTARPROPER
...
!for test 3  
!  rmin=log10(yieldtab%astar(1))
!  rmax=log10(yieldtab%astar(n11))
     ...
     if(npart1>0)then
        ...
        do jpart=1,npart1
           ...
           if( tp(ipart)/=0. .and. xx>0. .and. ihx>0 )then
              ...
              wxb = 1.-wxa
!                 wxb=(astarproper(ihx+1)-xx)/(astarproper(ihx+1)-astarproper(ihx))

              wya=(yy-yieldtab%zstar(ihy))  /(yieldtab%zstar(ihy+1)-yieldtab%zstar(ihy))
              wyb=1.-wya
!                 wyb=(yieldtab%zstar(ihy+1)-yy)/(yieldtab%zstar(ihy+1)-yieldtab%zstar(ihy))
              ...
              if (xx0>0.d0) then
                 ...
                 wxb0=1.-wxa0
!                    wxb0=(astarproper(ihx0+1)-xx0)  /(astarproper(ihx0+1)-astarproper(ihx0))
              else
                 ihx0=1
                 wxa0=0.
                 wxb0=0.
...
#endif
!  deallocate(Xeject,val3,astarproper) !for test3 - could this be the seg fault source? Xeject and val3 never used to be deallocated... why?
!111 format('   Entering thermal_feedback for level ',I2)
...
!for test 3  
  do iastar=1,n11
     astarproper(iastar) = yieldtab%astar(iastar)*aexp**2*(1d9*365.*24.*3600./scale_t) !bring it from supercomoving to proper time in Gyr by multiplying by aexp**2
! I HAD THIS BEFORE AS DIVIDING BY ASTAR CONV BUT TO INVERT FROM XX I SHOULD MULITPLY BY ALL EXCEPT SCALE_T
  end do
                 ...
                 do iSN=1,nSN
                    ...
                    if(dr_cell.le.dx_loc/2.0)then !same as above?
                       ...
                       if(metal)then                                                                                
                          ...
                          uold(ind_blast(iSN),imetal)=uold(ind_blast(iSN),imetal)-Zload*mload/vol_loc !average_SN doesn't normally update uold - should be unew?
                       endif                                                                                        
                       do ielt=1,nelt                                                                               
                          ceload=uold(ind_blast(iSN),ichem+ielt-1)/d                                                 
                          celoadSN(iSN,ielt)=( mload*ceload + ceSN(iSN,ielt)*mSN(iSN) ) / mloadSN(iSN)              
                          uold(ind_blast(iSN),ichem+ielt-1)=uold(ind_blast(iSN),ichem+ielt-1)-ceload*mload/vol_loc !average_SN doesn't normally update uold - should be unew?
                       enddo                                                                                        
