...
subroutine courant_fine(ilevel)
  ...
  ! Loop over active grids by vector sweeps
  ncache=active(ilevel)%ngrid
!$omp parallel do private(igrid,ngrid,imass_loc,iekin_loc,ieint_loc,idt_loc) reduction(+:mass_loc,ekin_loc,eint_loc), reduction(min:dt_loc)
  do igrid=1,ncache,nvector
     ngrid=MIN(nvector,ncache-igrid+1)
...
subroutine check_cons(ilevel)
  ...
  ! Loop over active grids by vector sweeps
  ncache=active(ilevel)%ngrid
!$omp parallel do private(igrid,ngrid,imass_loc,iekin_loc,ieint_loc) reduction(+:mass_loc,ekin_loc,eint_loc)
  do igrid=1,ncache,nvector
     ngrid=MIN(nvector,ncache-igrid+1)
