...
!#####################################################################
!#####################################################################
subroutine jeans_only_flag(ilevel)
  use amr_commons
  use hydro_commons
  implicit none
  integer::ilevel
  ! -------------------------------------------------------------------
  ! Lightweight Jeans-only refinement flag, called when holdback blocks
  ! all other criteria but jeans_bypass_holdback=.true.
  ! No neighbor gathering needed (jeans_length_refine is cell-local).
  ! -------------------------------------------------------------------
  integer::i,ncache,nok,iskip,iflag
  ...
  if(jeans_refine(ilevel)<=0.0)return

  ! Loop over active grids
  ncache=active(ilevel)%ngrid
  iflag=0
!$omp parallel do private(igrid,ngrid,ind_grid,ind_cell,ok,ind,iskip,i,nok) reduction(+:iflag)
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        ! Apply Jeans refinement only
        call jeans_length_refine(ind_cell,ok,ngrid,ilevel)

        ! Count newly flagged cells and set flag1
        nok=0
        do i=1,ngrid
           if(flag1(ind_cell(i))==0.and.ok(i))then
              nok=nok+1
...
end subroutine jeans_only_flag
!#####################################################################
!#####################################################################
!#####################################################################
!#####################################################################
