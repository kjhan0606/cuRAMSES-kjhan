...
! if(verbose)write(*,*)' +Entering kjhan_growspin'
  ...
  if(nsink .lt. nid*128) then
      call sub1_kjhan_growspin
  else
      call sub2_kjhan_growspin
  endif
  return
...
!###########################################################
!###########################################################
subroutine sub1_kjhan_growspin
  use amr_commons
  use pm_commons
  use hydro_commons
  implicit none
  ...
  if(verbose)write(*,*)' +Entering kjhan_growspin1'
...
end subroutine sub1_kjhan_growspin
!###########################################################
!###########################################################
!###########################################################
!###########################################################
subroutine sub2_kjhan_growspin
  use amr_commons
  use pm_commons
  use hydro_commons
  implicit none
  ...
  !----------------------------------------------------------------------
  ! local constants
  integer::isink,nisink,njsink,nwsink,displs(nid), recvcnt(nid),ierr
#ifndef WITHOUTMPI
  include 'mpif.h'
#endif
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v,scale_m
  ...
  real(dp), allocatable, dimension(:):: lbhspin, lspinmag, leps_sink
  ...
  allocate(lbhspin(1:nsink), lspinmag(1:nsink), leps_sink(1:nsink))
...
!$omp                 aac,jbh, jd,amod2,dmdeplete,mbh2,spinup,dm2) schedule(dynamic)
  do isink=nisink,njsink
     ...
     eps_sink(isink)=1d0-sqrt(1d0-2d0/(3d0*r_lso))
  enddo
  call MPI_Gatherv(bhspin(nisink),nwsink, MPI_DOUBLE_PRECISION, lbhspin, recvcnt,displs, &
                &   MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, ierr)
  call MPI_Bcast(lbhspin, nsink, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, ierr)
...
!$omp parallel do private(i)
  do i = 1, nsink
     spinmag(i) = lspinmag(i)
     bhspin(i) = lbhspin(i)
     eps_sink(i) = leps_sink(i)
  enddo

  deallocate(lspinmag, leps_sink, lbhspin)

end subroutine sub2_kjhan_growspin
!###########################################################
!###########################################################
!###########################################################
!###########################################################
