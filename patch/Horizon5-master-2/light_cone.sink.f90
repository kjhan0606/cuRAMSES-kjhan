...
!jhshin2    
  ...
  integer::igrid,jgrid,ipart,jpart,next_part,idim,icpu,ilevel
...
!  observer=(/Lbox*0.5,Lbox*0.5,Lbox*0.5/)
  ...
  if(myid==1)write(*,*)'Computing and dumping lightcone (sink particles)'
       ...
       if(npart1>0)then        
         ...
         do jpart=1,npart1
           ...
           if(idp(ipart).lt.0.and.tp(ipart).eq.0.d0) then
             r2=0.0
             do idim=1,ndim
               r2=r2+(xp(ipart,idim)-xsink(-idp(ipart),idim))**2
             enddo
             if(r2==0.0) then 
               npart2=npart2+1
             endif
       ...
       if(npart2>0) then 
         ...
         do jpart=1,npart2
           ...
           if(idp(ipart).lt.0.and.tp(ipart).eq.0.d0) then
             ...
             if(r2==0.0) then 
               ip=ip+1
               ind_part(ip)=-idp(ipart)
               if(ip==nvector)then
                 ! Lower left corner of 3x3x3 grid-cube
                 do idim=1,ndim
                   do i=1,ip
                     pos(idim,i)=xsink(ind_part(i),idim)*Lbox-minboxr(idim)
                     vel(idim,i)=vsink(ind_part(i),idim)
                     if(idim==1) then
					   ii8(i)=idsink(ind_part(i))
                       etc(1,i)=tsink(ind_part(i))
                       etc(2,i)=dMBH_coarse(ind_part(i))
                       etc(3,i)=dMEd_coarse(ind_part(i))
                     endif
                   end do
                 end do
                 !===========================================================================
             ...
             endif !sink particle condition 2
           endif ! sink particle condition 1
...
contains
    ...
    subroutine extend_arrays_if_needed_2()
    ...
    end subroutine extend_arrays_if_needed_2
