...
subroutine force_fine(ilevel,icount)
  ...
  integer::info,ibound,nx_loc,idim,jcount
...
#else
  integer ,target, allocatable, dimension(:,:)::Pind_grid,Pind_cell,Pind_cell_father
  integer ,pointer, dimension(:)::ind_grid,ind_cell,ind_cell_father
  real(dp),target,allocatable, dimension(:,:,:)::Pxx,Pff
  real(dp),pointer, dimension(:,:)::xx,ff
  common /omp_force_fine/ ind_grid,ind_cell,ind_cell_father, xx,ff
!$omp threadprivate(/omp_force_fine/)
...
!$omp end parallel
  allocate(Pind_grid(1:nvector,0:nthreads-1),Pind_cell(1:nvector,0:nthreads-1),Pind_cell_father(1:nvector,0:nthreads-1))
  allocate(Pxx(1:nvector,1:ndim,0:nthreads-1),Pff(1:nvector,1:ndim,0:nthreads-1))
!$omp parallel
  ind_grid=>Pind_grid(:,mythread)
  ind_cell=>Pind_cell(:,mythread)
  ind_cell_father=>Pind_cell_father(:,mythread)
  xx=>Pxx(:,:,mythread)
  ff=>Pff(:,:,mythread)
!$omp end parallel
#endif
...
!$omp parallel do private(igrid,ngrid,i,ind,iskip,idim) schedule(dynamic)
...
!$omp parallel do private(igrid,ngrid) 
  ...
  ! Loop over myid grids by vector sweeps
  ncache=active(ilevel)%ngrid
!$omp parallel do private(igrid,ngrid,i,ind,iskip,idim) reduction(+: epot_loc,rho_loc)
  do igrid=1,ncache,nvector
     ngrid=MIN(nvector,ncache-igrid+1)
     do i=1,ngrid
        ind_grid(i)=active(ilevel)%igrid(igrid+i-1)
...
#ifdef _OPENMP
  deallocate(Pind_grid,Pind_cell,Pind_cell_father)
  deallocate(Pxx,Pff)
