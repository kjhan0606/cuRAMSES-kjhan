...
!$omp Ly,Lz,Lmod,a1L,a2L,Lmodana,af) schedule(dynamic)
  do igrp=1, ngrp
...
! call MPI_BARRIER(MPI_COMM_WORLD, info) if(verbose) print *,'###7'

#ifdef _OPENMP
  deallocate(nparticles, ptrhead)
  deallocate(Pind_grid, Pind_grid_part,Pind_part)
#endif
! call MPI_BARRIER(MPI_COMM_WORLD, info) if(verbose) print *,'###8'
...
subroutine grow_bondi(ilevel)
  ...
  common /omp_g_b/ ind_grid, ind_part, ind_grid_part, velocity
...
!!$omp parallel do private(isink,density,c2mean, volume,v2mean,alpha,ZZ1,ZZ2,r_lso,epsilon_r,i)
  do isink=1,nsink
     density=0d0
     c2mean=0d0
     velocity=0d0
...
!!$     & /(scale_m/scale_t)
  ...
  do icpu=1,ncpu
     ...
     igrid=headl(icpu,ilevel)
     npart3 = numbl(icpu,ilevel)
!     call pthreadLinkedList(igrid, npart3, nthreads, nparticles, ptrhead, next)
!!$omp parallel private(npart3,igrid,jgrid,npart1,npart2,ipart,next_part,ip,ig, jpart) 
!     npart3 = nparticles(mythread)
!     igrid = ptrhead(mythread)
...
#ifdef _OPENMP
  deallocate(nparticles, ptrhead)
! deallocate(Pind_grid, Pind_grid_part, Pind_part,Pvelocity)
#endif
...
!$omp                 aac,jbh, jd,amod2,dmdeplete,mbh2,spinup,dm2)
  do isink=1,nsink
