...
subroutine output_frame()
  ...
  real(dp)::xtmp,ytmp,ztmp,smooth,theta_cam,phi_cam,fov_camera,dist_cam
  real(dp)::alpha=0,beta=0,smooth_theta
  ...
  real(dp)::minx=0,maxx=0,miny=0,maxy=0,minz=0,maxz=0
  real(dp)::xpc,ypc,zpc,d1,d2,d3,d4,l1,l2,l3,l4
 ...
 do proj_ind=1,LEN(trim(proj_axis)) 
  ...
  fov_camera = datan((delx/2d0)/focal_camera(proj_ind))
#if NDIM>2                 
  if(myid==1) then
     write(*,'(5A,F8.1,A,F8.1)') "Writing frame ", istep_str,' los=',proj_axis(proj_ind:proj_ind),' theta=',theta_cam*180./pi,' phi=',phi_cam*180./pi
  ...
  if(hydro) then
     ...
     do ilevel=levelmin,nlevelmax_frame
        ...
        if((shader_frame(proj_ind).eq.'cube').and.(.not.perspective_camera(proj_ind)))then
           ...
           do icube=1,8
              xtmp         = dcos(theta_cam)*xcube(icube)+dsin(theta_cam)*ycube(icube)
              ytmp         = dcos(theta_cam)*ycube(icube)-dsin(theta_cam)*xcube(icube)
              xcube(icube) = xtmp
              ycube(icube) = ytmp
              ytmp         = dcos(phi_cam)*ycube(icube)+dsin(phi_cam)*zcube(icube)
              ztmp         = dcos(phi_cam)*zcube(icube)-dsin(phi_cam)*ycube(icube)
              ycube(icube) = ytmp
              zcube(icube) = ztmp
           enddo
           minx = minval(xcube)
           ...
           do ind=1,twotondim
              ...
              do i=1,ngrid
                 if(ok(i))then
                    ...
                    xx(i,3) = xx(i,3)-zcen
                    ! Rotating
                    xtmp    = dcos(theta_cam)*xx(i,1)+dsin(theta_cam)*xx(i,2)
                    ytmp    = dcos(theta_cam)*xx(i,2)-dsin(theta_cam)*xx(i,1)
                    xx(i,1) = xtmp
                    xx(i,2) = ytmp
                    ytmp    = dcos(phi_cam)*xx(i,2)+dsin(phi_cam)*xx(i,3)
                    ztmp    = dcos(phi_cam)*xx(i,3)-dsin(phi_cam)*xx(i,2)
                    xx(i,2) = ytmp
                    xx(i,3) = ztmp
                    ...
                    if(proj_axis(proj_ind:proj_ind).eq.'x')then
                      ...
                      if(perspective_camera(proj_ind))then
                         ! if(shader_frame(proj_ind).eq.'cube')then
                            alpha  = datan(xx(i,2)/(dist_cam-xx(i,1)))
                            beta   = datan(xx(i,3)/(dist_cam-xx(i,1)))
                         ! endif
                    ...
                    elseif(proj_axis(proj_ind:proj_ind).eq.'y')then
                      ...
                      if(perspective_camera(proj_ind))then
                         ! if(shader_frame(proj_ind).eq.'cube')then
                            alpha  = datan(xx(i,1)/(dist_cam-xx(i,2)))
                            beta   = datan(xx(i,3)/(dist_cam-xx(i,2)))
                         ! endif
                         if(abs(alpha)/2d0.gt.fov_camera) cycle
                         if(abs(beta)/2d0.gt.fov_camera) cycle
                      ...
                      if(perspective_camera(proj_ind))then
                         ! if(shader_frame(proj_ind).eq.'cube')then
                            alpha  = datan(xx(i,1)/(dist_cam-xx(i,3)))
                            beta   = datan(xx(i,2)/(dist_cam-xx(i,3)))
                         ! endif
                         if(abs(alpha)/2d0.gt.fov_camera) cycle
                         if(abs(beta)/2d0.gt.fov_camera) cycle
                    ...
                    if(shader_frame(proj_ind).eq.'cube'.and.perspective_camera(proj_ind))then
                       ...
                       do icube=1,8
                          ...
                          xtmp         = dcos(alpha)*xcube(icube)+dsin(alpha)*zcube(icube)
                          ztmp         = dcos(alpha)*zcube(icube)-dsin(alpha)*xcube(icube)
                          xcube(icube) = xtmp
                          zcube(icube) = ztmp
                          ytmp         = dcos(beta)*ycube(icube)+dsin(beta)*zcube(icube)
                          ztmp         = dcos(beta)*zcube(icube)-dsin(beta)*ycube(icube)
                          ycube(icube) = ytmp
                          zcube(icube) = ztmp
...
666                          continue
  ...
  do j=1,npartmax
     ...
     zpf  = xp(j,3)-zcen
     ! Projection
     xtmp = dcos(theta_cam)*xpf+dsin(theta_cam)*ypf
     ytmp = dcos(theta_cam)*ypf-dsin(theta_cam)*xpf
     xpf  = xtmp
     ypf  = ytmp
     ytmp = dcos(phi_cam)*ypf+dsin(phi_cam)*zpf
     ztmp = dcos(phi_cam)*zpf-dsin(phi_cam)*ypf
     ...
     xpf  = xp(j,1)-xcen
     ypf  = xp(j,2)-ycen
     xtmp = dcos(theta_cam)*xpf+dsin(theta_cam)*xpf
     ytmp = dcos(theta_cam)*ypf-dsin(theta_cam)*ypf
     xpf  = xtmp
     ypf  = ytmp
     xpf  = xpf+xcen
     ypf  = ypf+xcen
