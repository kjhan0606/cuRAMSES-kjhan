!=======================================================================
!=======================================================================
! Modified by the HR5 team at KIAS
!=======================================================================
!=======================================================================
subroutine output_frame()
  ...
  real(kind=4),target, dimension(:,:,:,:),allocatable::Tdata_frame
  real(kind=4),target, dimension(:,:,:),allocatable::Tdens,Tvol

  real(kind=4),pointer, dimension(:,:,:,:)::thread_data_frame
  real(kind=4),pointer, dimension(:,:,:)::thread_dens,thread_vol
  common /omp_movie/ thread_data_frame, thread_dens, thread_vol
  integer:: nthreads, mythread
  common /omp_movie_threads/ nthreads, mythread
...
!$omp end parallel
  ...
  ! Allocate image
#ifdef SOLVERmhd
  allocate(Tdata_frame(1:nw_frame,1:nh_frame,0:NVAR+6,0:nthreads-1))
#else
  allocate(Tdata_frame(1:nw_frame,1:nh_frame,0:NVAR+2,0:nthreads-1))
#endif
  allocate(Tdens(1:nw_frame,1:nh_frame,0:nthreads-1))
  allocate(Tvol(1:nw_frame,1:nh_frame,0:nthreads-1))


!$omp parallel
  thread_data_frame=>Tdata_frame(:,:,:,mythread)
  thread_vol=>Tvol(:,:,mythread)
  thread_dens=>Tdens(:,:,mythread)
!$omp end parallel 
...
!$omp parallel do private(igrid)
        do igrid=1,ncache,nvector
           call suboutput_frame(ilevel, igrid, thread_data_frame, thread_vol, thread_dens)
        end do
        ! End loop over grids
     end do
  ...
  ! Loop over particles
!$omp parallel do private(j,xpf,ypf,zpf,xtmp,ytmp,ii,jj)
  do j=1,npartmax
     ...
     if(star) then
        if(tp(j).eq.0.) then
           if(mass_cut_refine>0.0.and.zoom_only) then
              if(mp(j)<mass_cut_refine) thread_data_frame(ii,jj,NVAR+5)=thread_data_frame(ii,jj,NVAR+5)+mp(j)
           else
              thread_data_frame(ii,jj,NVAR+5)=thread_data_frame(ii,jj,NVAR+5)+mp(j)
           endif
        else
           thread_data_frame(ii,jj,NVAR+6)=thread_data_frame(ii,jj,NVAR+6)+mp(j)
        endif
     else
        if(mass_cut_refine>0.0.and.zoom_only) then
           if(mp(j)<mass_cut_refine) thread_data_frame(ii,jj,NVAR+5)=thread_data_frame(ii,jj,NVAR+5)+mp(j)
     ...
     if(star) then
        if(tp(j).eq.0.) then
           if(mass_cut_refine>0.0.and.zoom_only) then
              if(mp(j)<mass_cut_refine) thread_data_frame(ii,jj,NVAR+1)=thread_data_frame(ii,jj,NVAR+1)+mp(j)
           else
              thread_data_frame(ii,jj,NVAR+1)=thread_data_frame(ii,jj,NVAR+1)+mp(j)
           endif
        else
           thread_data_frame(ii,jj,NVAR+2)=thread_data_frame(ii,jj,NVAR+2)+mp(j)
        endif
     else
        if(mass_cut_refine>0.0.and.zoom_only) then
           if(mp(j)<mass_cut_refine) thread_data_frame(ii,jj,NVAR+1)=thread_data_frame(ii,jj,NVAR+1)+mp(j)
...
#endif
        do ll = 0, nthreads-1
         data_frame(ii,jj,kk) = data_frame(ii,jj,kk) + Tdata_frame(ii,jj,kk,ll)
        enddo
        enddo
        enddo
        do ll = 0, nthreads-1
         dens(ii,jj,kk) = dens(ii,jj,kk) + Tdens(ii,jj,kk,ll)
        enddo
        do ll = 0, nthreads-1
         vol(ii,jj,kk) = vol(ii,jj,kk) + Tvol(ii,jj,kk,ll)
        enddo
     enddo
  enddo
  deallocate(Tdens, Tvol)
  deallocate(Tdata_frame)
...
subroutine suboutput_frame(ilevel, igrid, data_frame, dens, vol)
  use amr_commons
  use pm_commons
  use hydro_commons
#ifdef RT
  ...
  real(kind=8),dimension(:,:,:)::data_frame,data_frame_all
  real(kind=8),dimension(:,:)::dens,dens_all,vol,vol_all
  real(kind=4),dimension(:,:)::data_single
...
#ifdef RT
  character(len=100),dimension(1:NGROUPS) :: rt_moviefiles
  real(kind=8),dimension(:,:,:)::rt_data_frame,rt_data_frame_all
#endif  
...
end subroutine sub_output_frame
