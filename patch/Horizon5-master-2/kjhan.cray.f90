...
subroutine Build_Tree(node, particle, np)
  ...
  call  Make_Tree(node, particle,np, iworknode, iSpareNode0, iSpareNode)

end subroutine Build_Tree

recursive subroutine Make_Tree(node,particle,np,iworknode, iSpareNode0, iNextSpareNode)
  use Tree_commons
  implicit none
  integer:: np
...
20 continue
  ...
  do i = 0, 7
     if(Division(worknode, tmpnode(i).nump)) then
        j = inextjobnode
        k = iSpareNode
        call Make_Tree(node, particle,np, j, k, iSpareNode)
