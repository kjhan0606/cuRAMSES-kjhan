...
!$omp end parallel
  ...
  ! Loop over grids
  ncache=active(ilevel)%ngrid
!$omp parallel do private(igrid,ngrid,i,ind,iskip) firstprivate(ninc)
  do igrid=1,ncache,nvector
     ngrid=MIN(nvector,ncache-igrid+1)
     do i=1,ngrid
        ind_grid(i)=active(ilevel)%igrid(igrid+i-1)
...
!$omp parallel private(igrid,ngrid,ind,iskip,i,nnew,d,u,v,w,e,x,y,z,temp,d_jeans,d_thres,ind_cloud) firstprivate(index_sink,index_sink_tot) 
      nnew = iokthread(mythread)
      index_sink = index_sink + ioffsetthread
      index_sink_tot = index_sink_tot + ioffsetthread
!     print *, '##', myid,mythread, nnew, index_sink, index_sink_tot, ioffsetthread
   ! Create new sink particles
       do i=1 + ioffsetthread ,nnew+ioffsetthread
          index_sink=index_sink+1
          index_sink_tot=index_sink_tot+1
...
!$omp parallel do private(i)
 ...
 igrp = Do_FoF_Tree(nsink, psink, gsink, dx_min2, factG) 
  ...
  call MPI_BARRIER(MPI_COMM_WORLD, info)
  if(verbose) print *,'###2'
!$omp parallel do private(sisink,fisink,ijsink,isink,igrp,M1,M2,a1,a2,ax1,ay1,az1,ax2,ay2,az2,xx1,yy1,zz1,xx2,yy2,zz2,vvx1,vvy1,vvz1,vvx2,vvy2,vvz2,q,mu,a1mod,a2mod,a1a2,xx,yy,zz,xc,yc,zc,vxc,vyc,vzc,x1,y1,z1,x2,y2,z2,vx1,vy1,vz1,vx2,vy2,vz2,Lx1,Ly1,Lz1,Lx2,Ly2,Lz2,Lx,Ly,Lz,Lmod,a1L,a2L,Lmodana,af) schedule(dynamic)
  do igrp=1, ngrp
...
#ifdef _OPENMP
     call pthreadLinkedList(headl(icpu,ilevel), numbl(icpu,ilevel), nthreads, nparticles, ptrhead, next)
!$omp parallel private(npart3,igrid,jgrid,npart1, npart2,ipart,jpart,next_part) firstprivate(ip,ig)
     npart3 = nparticles(mythread)
     igrid = ptrhead(mythread)
     do jgrid=1,npart3
#else
...
!$omp end parallel
  ...
  do icpu=1,ncpu
     ...
     ip=0
     call pthreadLinkedList(headl(icpu,ilevel), numbl(icpu,ilevel), nthreads, nparticles, ptrhead, next)
!$omp parallel private(npart3,igrid,jgrid, npart1, npart2, ipart,jpart,next_part, isink, r2) firstprivate(ip,ig)
     npart3 = nparticles(mythread)
     igrid = ptrhead(mythread)
     do jgrid=1,npart3
       npart1=numbp(igrid)  ! Number of particles in the grid
...
!$omp parallel do private(isink)
  ...
  do icpu=1,ncpu
     ...
     ip=0
     call pthreadLinkedList(headl(icpu,ilevel), numbl(icpu,ilevel), nthreads, nparticles, ptrhead, next)
!$omp parallel private(igrid,npart3,jgrid, npart1, npart2,ipart,jpart,next_part) firstprivate(ip,ig)
     npart3 = nparticles(mythread)
     igrid = ptrhead(mythread)
     do jgrid=1,npart3
        npart1=numbp(igrid)  ! Number of particles in the grid
...
!!$     & /(scale_m/scale_t)
  ...
  do icpu=1,ncpu
     ...
     ip=0
     call pthreadLinkedList(headl(icpu,ilevel), numbl(icpu,ilevel), nthreads, nparticles, ptrhead, next)
!$omp parallel private(npart3,igrid,jgrid,npart1,npart2,ipart,next_part) firstprivate(ip,ig)
     npart3 = nparticles(mythread)
     igrid = ptrhead(mythread)
     do jgrid=1,npart3
   npart1=numbp(igrid)  ! Number of particles in the grid
...
!$omp end parallel
  ...
  do icpu=1,ncpu
     ...
     ip=0
     call pthreadLinkedList(headl(icpu,ilevel), numbl(icpu,ilevel), nthreads, nparticles, ptrhead, next)
!$omp parallel private(npart3,igrid,jgrid,npart1,ipart) firstprivate(ip,ig)
     npart3 = nparticles(mythread)
     igrid = ptrhead(mythread)
     do jgrid=1,npart3
        npart1=numbp(igrid)  ! Number of particles in the grid
...
!!$omp parallel do private(i,x,y,z,iAGN,dxx,dyy,dzz,dr_AGN,dr_cell,jtot,j_x,j_y,j_z,dzjet,drjet,psy,d,u,v,w,ekk,eint) schedule(dynamic)
          do i=1,ngrid
             if(ok(i))then
               ! Get gas cell position
               x=(xg(ind_grid(i),1)+xc(ind,1)-skip_loc(1))*scale
