...
!$omp threadprivate(/omp_threads/)
  integer ioffsetthread, joffsetthread
...
!  integer ,dimension(1:nvector)::ind_grid_new,ind_cell_new,ind_part
  ...
  common /openmpmk_sink/ ind_grid, ind_cell, ok,  ioffsetthread,joffsetthread
...
!$omp end parallel do
  ...
  ! Loop over grids
  ncache=active(ilevel)%ngrid
!$omp parallel do private(igrid,ngrid,i,ind,iskip, d, star_ratio, temp, d_jeans, d_thres, x,y,z,isink, dxx, dr_sink, dyy, dzz ) reduction(+: ntot)
  do igrid=1,ncache,nvector
     ngrid=MIN(nvector,ncache-igrid+1)
     do i=1,ngrid
       ind_grid(i)=active(ilevel)%igrid(igrid+i-1)
...
!$omp end parallel
  ...
  ! Loop over grids
  ncache=active(ilevel)%ngrid
!$omp parallel do private(igrid, ngrid, ind, iskip, nnew, ninc) schedule(dynamic)
  do igrid=1,ncache,nvector
     ngrid=MIN(nvector,ncache-igrid+1)
     do i=1,ngrid
        ind_grid(i)=active(ilevel)%igrid(igrid+i-1)
...
#else
              x_tmp(izero_myid+ninc+ioffsetthread,1)=(xg(ind_grid(i),1)+xc(ind,1)-skip_loc(1))*scale
              x_tmp(izero_myid+ninc+ioffsetthread,2)=(xg(ind_grid(i),2)+xc(ind,2)-skip_loc(2))*scale
              x_tmp(izero_myid+ninc+ioffsetthread,3)=(xg(ind_grid(i),3)+xc(ind,3)-skip_loc(3))*scale
              dens_tmp(izero_myid+ninc+ioffsetthread)=uold(ind_cell(i),1)
              flag_tmp(izero_myid+ninc+ioffsetthread)=1
...
#endif
#ifndef _OPENMP
! This original overlap algorithm is wrong and undeterministic.
  do i=1,ntot_tmp
  ...
  enddo
#else
!$omp parallel do private(i,x,y,z,dxx,dyy,dzz,dr_sink) schedule(dynamic)
  do i=1,ntot_tmp
     x=x_tmp(i,1);y=x_tmp(i,2);z=x_tmp(i,3)
     do j=i+1,ntot_tmp
        dxx=x-x_tmp(j,1)
...
!$omp parallel do private(igrid,ngrid,ind,iskip,i) reduction(+:nnew) schedule(static)
  ...
  ok_true = .true.
  call remove_free(ind_grid_new,nnew)
  call add_list(ind_part_cloud, ind_grid_cloud, ok_true, nnew)
...
!$omp parallel private(nnew,igrid,ngrid,ind,iskip,i,d,u,v,w,e,x,y,z,temp,d_jeans,d_thres)
  nnew = 0
...
!$omp parallel private(index_sink, index_sink_tot, igrid,ngrid,ind,iskip,i,nnew,d,u,v,w,e,x,y,z,temp,d_jeans,d_thres)
      nnew = iokthread(mythread)
	  index_sink = index_sink + ioffsetthread
	  index_sink_tot = index_sink_tot + ioffsetthread
   ! Create new sink particles
       do i=1,nnew
          index_sink=index_sink+1
          index_sink_tot=index_sink_tot+1
...
!$omp end parallel 
  ...
  !---------------------------------------------------------
  ncache=active(ilevel)%ngrid
!$omp parallel do private(igrid, ngrid, i, ind, iskip, d,u,v,w,e,bx1,by1,bz1,bx2,by2, bz2) schedule(dynamic)
  do igrid=1,ncache,nvector
     ngrid=MIN(nvector,ncache-igrid+1)
     do i=1,ngrid
       ind_grid(i)=active(ilevel)%igrid(igrid+i-1)
...
subroutine merge_sink(ilevel)
  ...
  common /omp_threads/ nthreads, mythread
!$omp threadprivate(/omp_threads/)
...
!$omp parallel do private(i)
 ...
 igrp = Do_FoF_Tree(nsink, psink, gsink, dx_min2, factG) 
  ...
  do ijsink=1,nsink
     GroupData(ijsink,1) = ngrp
     GroupData(ijsink,2) = 0
  enddo
  do ijsink=1,nsink
     GroupData(gsink(ijsink),1) = MIN(ijsink, GroupData(gsink(ijsink),1))
     GroupData(gsink(ijsink),2) = MAX(ijsink, GroupData(gsink(ijsink),2))
  enddo
!$omp parallel do private(isink,igrp,M1,M2,a1,a2,ax1,ay1,az1,ax2,ay2,az2,xx1,yy1,zz1,xx2,yy2,zz2,vvx1,vvy1,vvz1,vvx2,vvy2,vvz2,q,mu,a1mod,a2mod,a1a2,xx,yy,zz,xc,yc,zc,vxc,vyc,vzc,x1,y1,z1,x2,y2,z2,vx1,vy1,vz1,vx2,vy2,vz2,Lx1,Ly1,Lz1,Lx2,Ly2,Lz2,Lx,Ly,Lz,Lmod,a1L,a2L,Lmodana,af) schedule(dynamic)
  do igrp=1, ngrp
  sisink = GroupData(igrp,1)
  fisink = GroupData(igrp,2)
  do ijsink = sisink, fisink
  isink = psink(ijsink)
#else
  do isink=1,nsink
     igrp=gsink(isink)
...
#else
     xx=abs(xsink(isink,1)-xsink(int(oksink_new(igrp)),1))
	 if(xx > scale*xbound(1)*0.5) then
	    xx = scale*xbound(1) - xx
	 endif
#endif
...
#else
     yy=abs(xsink(isink,2)-xsink(int(oksink_new(igrp)),2))
	 if(yy > scale*xbound(2)*0.5) then
	    yy = scale*xbound(2) - yy
	 endif
#endif
...
#else
     zz=abs(xsink(isink,3)-xsink(int(oksink_new(igrp)),3))
	 if(zz > scale*xbound(3)) then
	   zz = scale*xbound(3) - zz
	 endif
#endif
...
!$omp parallel private(igrid,jgrid,npart1,npart2,npart3,ipart, jpart,next_part) firstprivate(ip,ig)
...
#ifdef _OPENMP
     call pthreadLinkedList(headl(icpu,ilevel), numbl(icpu,ilevel), nthreads, nparticles, ptrhead, next)
!$omp parallel private(jgrid,npart1, npart2,ipart,jpart,next_part) firstprivate(ip,ig)
     npart3 = nparticles(mythread)
	 igrid = ptrhead(mythread)
	 do jgrid=1,npart3
#else
...
!$omp threadprivate(/openmp_b_h/)
  ...
  common /openmp_threads/ mythread, nthreads
!$omp threadprivate(/openmp_threads/)
#endif
...
#ifdef _OPENMP
     call pthreadLinkedList(headl(icpu,ilevel), numbl(icpu,ilevel), nthreads, nparticles, ptrhead, next)
!$omp parallel private(jgrid, npart1, npart2, ipart,jpart,next_part, isink, r2) firstprivate(ip,ig)
     npart3 = nparticles(mythread)
	 igrid = ptrhead(mythread)
	 do jgrid=1,npart3
#else
...
!$omp parallel do private(isink)
  ...
  ! Loop over cpus
  do icpu=1,ncpu
     if(numbl(icpu,ilevel).le.0) goto 103
     igrid=headl(icpu,ilevel)
     ig=0
     ip=0
#ifdef _OPENMP
...
subroutine grow_bondi(ilevel)
  ...
  integer,dimension(1:nvector),save::ind_grid,ind_part,ind_grid_part
#else
  integer,allocatable, target::Pind_grid(:,:),Pind_part(:,:),Pind_grid_part(:,:)
  integer,pointer::ind_grid(:),ind_part(:),ind_grid_part(:)
  common /omp_g_b/ ind_grid, ind_part, ind_grid_part
!$omp threadprivate(/omp_g_b/)
#endif
...
#else
  real(dp), target, allocatable ::Pvelocity(:,:)
  real(dp), pointer ::velocity(:)
  common /grow_b/ velocity
!$omp threadprivate(/grow_b/)
  integer:: mythread, nthreads,npart3
  common /openmp_threads/ mythread, nthreads
!$omp threadprivate(/openmp_threads/)
  integer, dimension(:), allocatable:: nparticles, ptrhead
...
!$omp parallel
  ...
  nthreads = omp_get_num_threads()
!$omp end parallel
  allocate(Pvelocity(1:3, 0: nthreads-1))
  allocate(Pind_grid(1:nvector, 0: nthreads-1))
  allocate(Pind_part(1:nvector, 0: nthreads-1))
  allocate(Pind_grid_part(1:nvector, 0: nthreads-1))
  allocate(nparticles(0:nthreads-1), ptrhead(0:nthreads-1))
!$omp parallel
  velocity => Pvelocity(:, mythread)
  ind_grid => Pind_grid(:, mythread)
  ind_part => Pind_part(:, mythread)
  ind_grid_part => Pind_grid_part(:, mythread)
!$omp end parallel
...
!$omp parallel do private(isink,density,c2mean, volume,v2mean,alpha,ZZ1,ZZ2,r_lso,epsilon_r)
  do isink=1,nsink
     density=0d0
     c2mean=0d0
     velocity=0d0
...
#ifdef _OPENMP
     call pthreadLinkedList(headl(icpu,ilevel), numbl(icpu,ilevel), nthreads, nparticles, ptrhead, next)
!$omp parallel private(igrid,jgrid,npart1,npart2,ipart,next_part) firstprivate(ip,ig)
     npart3 = nparticles(mythread)
	 igrid = ptrhead(mythread)
	 do jgrid=1,npart3
#else
      ...
      uold(indp(j),imetal)=d*z
        ...
        if(drag)then
           ...
           do idim=1,ndim
              ...
              vsink_new(isink,idim)=vsink_new(isink,idim)+dvdrag*mp(ind_part(j))!/msink(isink)
           enddo
...
#ifdef _OPENMP
     call pthreadLinkedList(headl(icpu,ilevel), numbl(icpu,ilevel), nthreads, nparticles, ptrhead, next)
!$omp parallel private(igrid,jgrid,npart1,ipart) firstprivate(ip,ig)
     npart3 = nparticles(mythread)
	 igrid = ptrhead(mythread)
	 do jgrid=1,npart3
#else
...
subroutine quenching(ilevel)
  ...
  ! Loop over grids
!$omp parallel do private(i,igrid,npart1,npart2, str_d, tot_m,ave_u, ave_v, ave_w,sig_u,sig_v,sig_w,ipart,jpart,next_part,ind,iskip,ind_cell)
  do i=1,active(ilevel)%ngrid
...
!$omp parallel do private(isink,mdisc,dm,chi,dmcoarse, dmini,mbh,ax2,ay2,az2,jsinknorm, amod,ZZ1,ZZ2,r_lso,epsilon_r, chieps,mass_8,t_nu1,rwarp,rsg,msg,dmacc,ax1,ay1,az1,bhspinnorm,theta,aac,jbh, jd,amod2,dmdeplete,mbh2,spinup,dm2)
  do isink=1,nsink
...
!$omp parallel do private(i,x,y,z,dxx,dyy,dzz,dr_AGN,dr_cell,jtot,j_x,j_y,j_z,dzjet,drjet,psy,d,u,v,w,ekk,eint)
      do i=1,ngrid
         if(ok(i))then
       ! Get gas cell position
       x=(xg(ind_grid(i),1)+xc(ind,1)-skip_loc(1))*scale
...
subroutine AGN_blast(xAGN,vAGN,dMsmbh_AGN,dMBH_AGN,dMEd_AGN,mAGN,ZAGN,jAGN,ind_blast,vol_gas &
  ...
  real(dp),dimension(1:nAGN)::EAGN,X_radio,psy_norm,dMsmbh_AGN,dMBH_AGN,dMEd_AGN,EsaveAGN,spinmagAGN,eps_AGN
  real(dp),dimension(1:nAGN,1:3)::xAGN,vAGN,jAGN
  integer ,dimension(1:nAGN)::ind_blast,iAGN_myid
  ...
  logical, pointer:: ok(:)
  integer,pointer::ind_grid(:),ind_cell(:)
  common /omp_a_b/ ok, ind_grid, ind_cell
...
!$omp parallel do private(iAGN, epsilon_r,eff_mad)
  ...
  do ilevel=levelmin,nlevelmax
   ...
   xc(ind,3)=(dble(iz)-0.5D0)*dx
     ...
     ! Loop over grids
     ncache=active(ilevel)%ngrid
!$omp parallel do private(igrid,ngrid,ind,iskip,i,x,y,z,iAGN,dxx,dyy,dzz,dr_AGN,ekk,u,idim,etot,eint,T2_1,jtot,j_x,j_y,j_z,dzjet,drjet,psy,v,w,ekkold, d_gas,d,T2_2) schedule(dynamic)
     do igrid=1,ncache,nvector
   ngrid=MIN(nvector,ncache-igrid+1)
   do i=1,ngrid
      ind_grid(i)=active(ilevel)%igrid(igrid+i-1)
...
#ifdef _OPENMP
  deallocate(Pind_grid, Pind_cell, Pok)
#endif
