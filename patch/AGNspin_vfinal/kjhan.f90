...
logical function pfof_open(node, particle,np, inow, ptr, foflink, boxsize) 
  ...
  TYPE(Tree_Node), dimension(1:np):: node
  TYPE(Tree_Particle),  dimension(1:np):: particle
  real, dimension(1:3):: boxsize
  integer:: inow, ptr
  real:: foflink,tmpx,tmpy,tmpz,dist,diff
...
integer function FoF_link(gndx, node, particle, np, foflink , linked, gindx, boxsize, factG) result(ncount)
  ...
  TYPE(Tree_Node), dimension(1:np):: node
  TYPE(Tree_Particle),  dimension(1:np):: particle
  integer, dimension(1:np):: linked
  integer:: i,j,k,ptr,optr, nptr,now,gndx,gindx,np
  real, dimension(3):: boxsize
  real:: foflink2,tmpx,tmpy,tmpz, foflink,rr
     ...
     do while(ptr .ne. 0)
        if(ptr.lt. 0) then
           ...
           if(particle(ptr)%included .eq. .true.) then
              ...
              if(tmpy.ge.boxsize(2)*0.5) tmpy = boxsize(2) - tmpy
              if(tmpz.ge.boxsize(3)*0.5) tmpz = boxsize(3) - tmpz
              rr = tmpx*tmpx + tmpy*tmpy + tmpz*tmpz
...
integer function Do_FoF_Tree(numsink, qsink, rsink, dx_min2, factG) result(gindx)
#ifdef _KJHAN_TEST
  use AA
#else
  use pm_commons
  ...
  TYPE(Tree_Node), allocatable, dimension(:):: node
  TYPE(Tree_Particle), allocatable, dimension(:):: particle
  integer, allocatable, dimension(:):: linked
  integer,dimension(1:numsink)::qsink,rsink
  ...
  allocate(node(1:numsink), particle(1:numsink), linked(1:numsink))
  do i = 1, numsink
     indx = qsink(i)
  ...
  do i = 1, numsink
     if(particle(i)%included .eq. .false.) then
        ...
        do j = 1, gnum
           qsink(icount+j) = linked(j)
           rsink(icount+j) = gindx
        enddo
        icount = icount + gnum
     endif
  enddo
...
subroutine Build_Tree(node, particle, np)
  ...
  iSpareNode = Make_Tree(node, particle,np, -1, -2)
...
20 continue
  ...
  do i = 0, 7
     if(Division(worknode, tmpnode(i).nump)) then
        iSpareNode = Make_Tree(node, particle,np, inextjobnode, iSpareNode)
...
subroutine pthreadLinkedList(ipart,nworks,nthreads,nparticles,ptrhead,nextlink)
   ...
   integer, dimension(:):: nextlink
   integer, dimension(0:nthreads-1):: nparticles, ptrhead
