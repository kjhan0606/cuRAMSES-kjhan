...
subroutine thermal_feedback(ilevel)
  ...
  common /openmp_t_feedback/ ind_grid,ind_part,ind_grid_part
!$omp threadprivate(/openmp_t_feedback/)
...
!$omp end parallel
  ...
  ! Loop over cpus
  do icpu=1,ncpu
     if(numbl(icpu,ilevel).le.0) goto 23
...
#ifdef _OPENMP
   deallocate(Pind_grid, Pind_part, Pind_grid_part)
...
#ifdef _OPENMP
  integer::ithread, nthreads,mythread,icount,nwork,jcount,npart3
  integer:: t_offset,jSN,jp
  integer, dimension(:), allocatable:: nparticles, ptrhead
  common /openmpthreads/nthreads,mythread
!$omp threadprivate(/openmpthreads/)
  integer, dimension(:), allocatable:: iSNthread,ncountsperthread
#endif
...
!$omp parallel
  ...
  nthreads = omp_get_num_threads()
!$omp end parallel
  allocate(iSNthread(0:nthreads-1))
  allocate(ncountsperthread(0:nthreads-1))
  allocate(nparticles(0:nthreads-1), ptrhead(0:nthreads-1))
  iSNthread = 0
  ncountsperthread = 0
#endif
  ...
  ! Loop over levels
  do icpu=1,ncpu
     if(numbl(icpu,ilevel).le.0) goto 10
     igrid=headl(icpu,levelmin)
#ifdef _OPENMP
     iSN = 0
	 call pthreadLinkedList(headl(icpu,ilevel), numbl(icpu,ilevel), nthreads, nparticles, ptrhead, next)
  ! Loop over cpus
!$omp parallel private(igrid,jgrid,npart1,npart2,ipart,jpart,next_part) reduction(+:iSN)
     npart3 = nparticles(mythread)
	 igrid = ptrhead(mythread)
	 do jgrid=1,npart3
#else
...
#else
        iSN = iSN + npart2
		iSNthread(mythread) = iSNthread(mythread) + npart2
#endif
        igrid=next(igrid)   ! Go to next grid
     end do
#ifdef _OPENMP
...
10  end do
  ...
  ip=0
  do icpu=1,ncpu
     if(numbl(icpu,levelmin).le.0) goto 82
     igrid=headl(icpu,levelmin)
#ifdef _OPENMP
     call pthreadLinkedList(headl(icpu,ilevel), numbl(icpu,ilevel), nthreads, nparticles, ptrhead, next)
!$omp parallel private(npart1, jpart, next_part, ipart,  jSN, jp, igrid, t_offset) firstprivate(ip)
     npart3 = nparticles(mythread)
	 igrid = ptrhead(mythread)
	 if(mythread .eq.0) then
	   t_offset = 0 + iSN
	 else
	   t_offset = sum(iSNthread(0:mythread-1)) + iSN
	 endif
	 do jgrid=1,npart3
#else
     do jgrid=1,numbl(icpu,levelmin)
        ...
        if(npart1>0)then
           ...
           do jpart=1,npart1
              ...
              if(idp(ipart).le.0.and. tp(ipart).lt.(current_time-t0))then
                 ...
                 ind_part(ip)=ipart
#else
                 ncountsperthread(mythread) = ncountsperthread(mythread) + 1
                 jSN = t_offset + ncountsperthread(mythread)
                 xSN(jSN,1)=xp(ipart,1)
                 xSN(jSN,2)=xp(ipart,2)
                 xSN(jSN,3)=xp(ipart,3)
                 vSN(jSN,1)=vp(ipart,1)
                 vSN(jSN,2)=vp(ipart,2)
                 vSN(jSN,3)=vp(ipart,3)
                 mSN(jSN)=mp(ipart)
                 sSN(jSN)=dble(-idp(ipart))*mstar
                 if(metal)ZSN(jSN)=zp(ipart)
				 jp = (t_offset - iSN) + ncountsperthread(mythread)
                 ind_grid(jp)=igrid
                 ind_part(jp)=ipart
#endif
              endif
...
subroutine average_SN(xSN,vol_gas,dq,ekBlast,ind_blast,nSN)
  ...
  logical ,dimension(1:nvector),save::ok
#else
  integer,allocatable, target::Pind_grid(:,:),Pind_cell(:,:)
  integer,pointer::ind_grid(:),ind_cell(:)
  logical ,target, allocatable::Pok(:,:)
  logical ,pointer::ok(:)
  common /omp_a_SN/ ind_grid, ind_cell, ok
!$omp threadprivate(/omp_a_SN/)
...
!$omp parallel do private(igrid,ngrid,i,ind,iskip,x,y,z,iSN,dxx,dyy,dzz,dr_SN,dr_cell,u,v,w) schedule(dynamic)
  ...
  ekBlast=ekBlast_all
#endif
!$omp parallel do private(iSN, u2, v2, w2) schedule(dynamic)
  do iSN=1,nSN
...
subroutine Sedov_blast(xSN,vSN,mSN,sSN,ZSN,indSN,vol_gas,dq,ekBlast,nSN)
  ...
  logical ,target, allocatable::Pok(:,:)
  logical ,pointer::ok(:)
  common /omp_s_b/ ind_grid, ind_cell, ok
!$omp threadprivate(/omp_s_b/)
  integer mythread, nthreads,nwork,icount,jcount
  common /omp_threads/ mythread, nthreads
!$omp threadprivate(/omp_threads/)
#endif
...
!$omp parallel do private(iSN) 
  ...
  do ilevel=levelmin,nlevelmax
     ...
     ! Loop over grids
     ncache=active(ilevel)%ngrid
!$omp parallel do private(igrid,ngrid,i,ind, iskip,iSN,x,y,z,dxx,dyy,dzz,dr_SN,u,v,w) schedule(dynamic)
     do igrid=1,ncache,nvector
        ngrid=MIN(nvector,ncache-igrid+1)
        do i=1,ngrid
           ind_grid(i)=active(ilevel)%igrid(igrid+i-1)
...
!$omp parallel do private(iSN,u,v,w) schedule(dynamic)
  do iSN=1,nSN
     if(vol_gas(iSN)==0d0)then
        u=vSN(iSN,1)
        v=vSN(iSN,2)
