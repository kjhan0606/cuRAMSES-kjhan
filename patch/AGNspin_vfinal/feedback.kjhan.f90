...
subroutine thermal_feedback(ilevel)
  ...
  common /othreads/ mythread
!$omp threadprivate(/othreads/)
...
!$omp parallel private(subnump,igrid,jgrid)
     subnump = nparticles(mythread)
     igrid = ptrhead(mythread)
     call sub_thermal_feedback(icpu,ilevel, igrid,subnump)
!$omp end parallel
12  enddo
...
!################################################################
!################################################################
subroutine sub_thermal_feedback(icpu,ilevel,igrid,subnump)
  use pm_commons
  use amr_commons
  implicit none
  integer::ilevel,subnump
  !------------------------------------------------------------------------
  ! This routine computes the thermal energy, the kinetic energy and 
  ! the metal mass dumped in the gas by stars (SNII, SNIa, winds).
  ! This routine is called every fine time step.
