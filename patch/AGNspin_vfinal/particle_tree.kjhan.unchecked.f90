...
subroutine make_tree_fine(ilevel)
  ...
  common /openmp_mk_tree_fine/ ind_grid,ind_part,ind_grid_part
!$omp threadprivate(/openmp_mk_tree_fine/)
  integer ::ithread, icount,nwork,jcount,npart3
...
!$omp parallel private(npart1, npart3,igrid, jgrid,ipart,jpart,next_part) firstprivate(ip,ig)
     npart3 = nparticles(mythread)
	 igrid = ptrhead(mythread)
	 print *, myid, mythread, npart3, igrid, numbl(icpu, ilevel)
	 do jgrid=1,npart3
#else
     ! Loop over grids
...
subroutine kill_tree_fine(ilevel)
  ...
  integer,target,allocatable::Pind_grid(:,:),Pind_part(:,:),Pind_grid_part(:,:)
  integer, pointer:: ind_grid(:), ind_part(:), ind_grid_part(:)
  common /openmp_k_tree_fine/ ind_grid,ind_part,ind_grid_part
!$omp threadprivate(/openmp_k_tree_fine/)
  integer ::ithread, icount,nwork,jcount,npart3
  integer:: nthreads,mythread
  common /openmpthreads/ nthreads, mythread
  integer, dimension(:), allocatable:: nparticles, ptrhead
...
#ifdef _OPENMP
     call pthreadLinkedList(headl(icpu,ilevel), numbl(icpu,ilevel), nthreads, nparticles, ptrhead, next)
!$omp parallel private(npart1, igrid, jgrid,ipart,jpart,next_part) firstprivate(ip,ig)
     npart3 = nparticles(mythread)
	 igrid = ptrhead(mythread)
     do jgrid=1,npart3
#else
...
#ifdef _OPENMP
  deallocate(Pind_grid,Pind_cell,Pind_grid_son,Pok)
#endif
...
! This routine can not be pthreaded because the global variable of reception.
