subroutine move_fine(ilevel)
  ...
  common /openmpthreads1/ mythread
!$omp threadprivate(/openmpthreads1/)
...
!$omp parallel private(subnump,igrid)
  ...
  call sub_move_fine(ilevel, igrid,subnump)
!$omp end parallel
...
!###########################################################
!###########################################################
subroutine sub_move_fine(ilevel,kgrid,subnump)
  use amr_parameters
  use amr_commons
  use pm_commons
  implicit none
  ...
  include 'mpif.h' 
#endif
  integer, intent(in):: kgrid, ilevel, subnump
  !----------------------------------------------------------------------
  ! Update particle position and time-centred velocity at level ilevel. 
  ! If particle sits entirely in level ilevel, then use fine grid force
  ! for CIC interpolation. Otherwise, use coarse grid (ilevel-1) force.
...
!$omp critical 
 ...
 ! Compute parent cell position
  ...
  do idim=1,ndim
     if(static)then
        do j=1,np
           xp(ind_part(j),idim)=xp(ind_part(j),idim)
        end do
     else
        do j=1,np
           xp(ind_part(j),idim)=xp(ind_part(j),idim)+new_vp(j,idim)*dtnew(ilevel)
        end do
     endif
  end do
!$omp end critical 
