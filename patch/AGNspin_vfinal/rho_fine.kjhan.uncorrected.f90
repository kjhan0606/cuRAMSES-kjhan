...
subroutine rho_fine(ilevel,icount)
  ...
  !------------------------------------------------------------------
  integer::iskip,icpu,ind,jnd,i,info,nx_loc,ibound,idim
...
#else
           jnd=active(ilevel)%igrid(i) + iskip
           rho_top(jnd)=rho_top(father(jnd-iskip))
           rho(jnd)=rho(jnd)+  rho_top(jnd)
#endif
        end do
     end do
  endif
...
!$omp parallel do private(i, scalar)
              do i=1,active(ilevel)%ngrid
...
!$omp parallel do private(ibound,ind,iskip,i)
  ...
  if(hydro)then
     ...
     ncache=active(ilevel)%ngrid
!$omp parallel do private(igrid,ngrid,i) schedule(dynamic)
     do igrid=1,ncache,nvector
