...
#else
  integer,target,dimension(:,:),allocatable::Pind_grid,Pind_cell, Pind_leaf
  integer,pointer, dimension(:)::ind_grid,ind_cell,ind_leaf
  common /omp_cour_fine/ ind_grid,ind_cell, ind_leaf
!$omp threadprivate(/omp_cour_fine/)
  integer:: nthreads, mythread
  common /ompthreads/ nthreads, mythread
!$omp threadprivate(/ompthreads/)
  real(dp),target,allocatable,dimension(:,:,:)::Puu
  real(dp),target,allocatable,dimension(:,:,:)::Pgg
  real(dp),pointer, dimension(:,:)::uu
  real(dp),pointer, dimension(:,:)::gg
  common /omp_cour_fine2/ uu,gg
!$omp threadprivate(/omp_cour_fine2/)
#endif
...
!$omp end parallel
  ...
  allocate(Pind_leaf(1:nvector,0:nthreads-1))
  allocate(Puu(1:nvector,1:nvar,0:nthreads-1))
  allocate(Pgg(1:nvector,1:ndim,0:nthreads-1))
!$omp parallel
  ind_grid=>Pind_grid(:,mythread)
  ind_cell=>Pind_cell(:,mythread)
  ind_leaf=>Pind_leaf(:,mythread)
  uu=>Puu(:,:,mythread)
  gg=>Pgg(:,:,mythread)
!$omp end parallel
#endif
...
!$omp parallel do private(igrid,ngrid,i,ind,iskip,nleaf, ivar,idim) reduction(+:mass_loc,eint_loc,ekin_loc), reduction(MIN: dt_loc)
...
#ifdef _OPENMP
  deallocate(Pind_grid, Pind_cell, Pind_leaf)
  deallocate(Puu, Pgg)
...
!  integer,target,dimension(:,:),allocatable::Pind_grid,Pind_cell, Pind_leaf
!  integer,pointer, dimension(:)::ind_grid,ind_cell,ind_leaf
!  common /omp_check_cons/ ind_grid,ind_cell, ind_leaf
!!$omp threadprivate(/omp_check_cons/)
!  integer:: nthreads, mythread
!  common /ompthreads/ nthreads, mythread
!!$omp threadprivate(/ompthreads/)
!  real(dp),target,allocatable,dimension(:,:,:)::Puu
!  real(dp),pointer, dimension(:,:)::uu
!  common /omp_check_cons2/ uu
!!$omp threadprivate(/omp_check_cons2/)
!#endif
...
!#endif
  ...
  ! Loop over active grids by vector sweeps
  ncache=active(ilevel)%ngrid
!!$omp parallel do private(igrid,ngrid,i,ind,iskip,nleaf, ivar) reduction(+:mass_loc,eint_loc,ekin_loc) 
  do igrid=1,ncache,nvector
     ngrid=MIN(nvector,ncache-igrid+1)
     do i=1,ngrid
        ind_grid(i)=active(ilevel)%igrid(igrid+i-1)
